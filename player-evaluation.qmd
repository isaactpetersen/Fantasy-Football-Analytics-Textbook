...
Many aspects of a player's athletic profile, including tests of speed (40-yard dash), strength (bench press), agility (2-yard shuttle run; three cone drill), and jumping ability (vertical jump; broad jump) are available from the National Football League (NFL) Combine, which is especially relevant for evaluating rookies.
...
Compared to tests of speed, power, and agility at the NFL Combine, collegiate performance is a stronger predictor of performance in the NFL [@Lyons2011].
That is, previous sports performance is the best predictor of future performance [for a review, see @DenHartigh2018].
Thus, it is important to consider a player's past performance.
