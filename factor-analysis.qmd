...
A path diagram of the EFA model in @fig-efaPathDiagram-3factor was created using the `lavaanPlot2()` function of the `lavaanPlot` package [@R-lavaanPlot].
...
Here is a modification of the 3-factor model with correlated residuals:
...
```{r}
efa3factorModified_factorScores[which.max(efa3factorModified_factorScores[,"F1"]),]
```

```{r}
dataForFA[which.max(efa3factorModified_factorScores[,"F1"]), c("player_display_name", "season", factor1vars)]
```

```{r}
efa3factorModified_factorScores[which.max(efa3factorModified_factorScores[,"F2"]),]
```

```{r}
dataForFA[which.max(efa3factorModified_factorScores[,"F2"]), c("player_display_name", "season", factor2vars)]
```

```{r}
efa3factorModified_factorScores[which.max(efa3factorModified_factorScores[,"F3"]),]
```

```{r}
dataForFA[which.max(efa3factorModified_factorScores[,"F3"]), c("player_display_name", "season", factor3vars)]
```
...
```{r}
cfa3factorModified_factorScores[which.max(cfa3factorModified_factorScores[,"F1"]),]
```

```{r}
dataForFA[which.max(cfa3factorModified_factorScores[,"F1"]), c("player_display_name", "season", factor1vars)]
```

```{r}
cfa3factorModified_factorScores[which.max(cfa3factorModified_factorScores[,"F2"]),]
```

```{r}
dataForFA[which.max(cfa3factorModified_factorScores[,"F2"]), c("player_display_name", "season", factor2vars)]
```

```{r}
cfa3factorModified_factorScores[which.max(cfa3factorModified_factorScores[,"F3"]),]
```

```{r}
dataForFA[which.max(cfa3factorModified_factorScores[,"F3"]), c("player_display_name", "season", factor3vars)]
```
