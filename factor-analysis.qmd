...
dataMerged <- full_join(
  player_stats_seasonal_avgPerGame,
...
```{r}
#efaVars <- c(
#  "completionsPerGame","passing_yardsPerGame","passing_tdsPerGame",
...
#faVars <- unique(c(cfaVars, efaVars))
...
#dataForFA[faVars] <- scale(dataForFA[faVars])
```
...
```{r}
lavaan::lavPredict(efa3factor_fit)
```
...
```{r}
lavaan::lavPredict(efa3factorModified_fit)
```
...
```{r}
lavaan::lavPredict(cfa1factor_fit)
```
...
```{r}
lavaan::lavPredict(cfa2factor_fit)
```
...
```{r}
lavaan::lavPredict(cfa3factor_fit)
```
...
```{r}
lavaan::lavPredict(cfa3factorModified_fit)
```
