...
Football is the most widely watched sport in the United States [@Jones2024\; archived at <https://perma.cc/X2UG-RAAK>; @statistica2023\; archived at <https://perma.cc/JNU6-S96A>].
...
@fig-football-intro is a video that provides a brief introduction to American football [@Ly2015]:

::: {#fig-football-intro}

{{< video images/football.mp4 >}}
...
@Ly2015 provides a video of a brief introduction to American football.
...
A pass defended occurs when a defensive player knocks down the ball in the air so that the intended receiver cannot catch the ball.
...
The current record for the longest field goal is 68 yards (by Cam Little in 2025).
...
### Advanced Metrics {#sec-footballAdvancedMetrics}

indices of [skill](#sec-skill):

- yards after the catch (YAC)—how many yards after the catch a player has
- broken tackles / missed tackles forced—the number of tackles a ball carrier or receiver causes defenders to miss

indices of [opportunity](#sec-evalSituation):

- carries—the number of plays the player runs the ball
- routes run—the number of plays the player runs a route (i.e., an attempted pass pattern to receive a throw)
- targets—the number of plays the player is targeted on a pass attempt
- route participation—the percentage of passing plays (dropbacks) the team runs in which the player runs a route; $\frac{\text{routes run}}{\text{dropback plays}}$
- targets per route run (TPRR; target rate)—targets divided by routes run (i.e., the percentage of times the player is targeted when running a route); $\frac{\text{targets}}{\text{routes run}}$
- target share—the player's number of targets divided by the team's number of targets (i.e., the percentage of a team's pass targets directed to the player); $\frac{\text{player's targets}}{\text{team's targets}}$
- air yards (AY)—the total distance downfield from the line of scrimmage to the location of a target on all pass attempts (for a Quarterback) or targets (for a Wide Receiver/Tight End)
    - completed air yards (CAY)—the total distance downfield from the line of scrimmage to the location of a target on all completed pass attempts (for a Quarterback) or receptions (for a Wide Receiver/Tight End); $\text{receiving yards} = \text{completed air yards} + \text{yards after the catch}$
    - unrealized (or incomplete) air yards (UAY)—the total distance downfield from the line of scrimmage to the location of a target on all incomplete pass attempts (for a Quarterback) or incomplete targets (for a Wide Receiver/Tight End)
- air yards share—the player's air yards divided by the team's air yards (i.e., the percent of a team's air yards that target the player); $\frac{\text{player's air yards}}{\text{team's air yards}}$
- average depth of target (ADOT)—the average distance downfield per target (i.e., average air-yard distance per target); $\frac{\text{air yards}}{\text{targets}}$

indices of [efficiency](#sec-evalHistoricalEfficiency):

- expected points added (EPA)—how much each play changes a team's scoring outcome
- completion percentage over expected (CPOE)—how much higher (or lower) a player's (e.g., Quarterback's) completion percentage is relative to what would be expected based on the types and difficulties of passes they attempted (e.g., depth of throw, distance to sideline, proximity of defenders)
- rushing yards over expected per attempt (RYOE)—how many rushing yards a player (e.g., Running Back) gets compared to what the player would be expected to get based on the blocking and defensive context of the play (e.g., positioning)
- yards after catch over expected (YACOE)—how many yards after the catch a player generates compared to what would be expected based on the circumstances of the catch
- missed tackles forced rate—how often a player (e.g., Runnning Back) makes a defender miss divided by the number of touches the player gets; $\frac{\text{missed tackles forced}}{\text{touches}}$
- opportunity share—a player's carries plus targets divided by the team's total number of carries and targets; $\frac{\text{player's carries} + \text{player's targets}}{\text{team's carries} + \text{team's targets}}$
- yards per route run (YPRR)—receiving yards divided by routes run; $\frac{\text{receiving yards}}{\text{routes run}}$
...
It is estimated that around 62 million people play fantasy sports [@FSGA2023\; archived at <https://perma.cc/9PB8-ZDJJ>], of whom around 29 million play fantasy football [@statistica2023a\; archived at <https://perma.cc/8YSN-UUNT>].
...
@fig-fantasy-football-intro is a video that provides a brief introduction to fantasy football [@NFLFantasy2020]:

::: {#fig-fantasy-football-intro}

{{< video images/fantasyFootball.mp4 >}}
...
@NFLFantasy2020 provides a video of a brief introduction to fantasy football.
...
: Offensive Players in the Starting Lineup {#tbl-lineupOffensePDF}
...
: Common Scoring Settings for Offensive Players {#tbl-scoringOffensePDF}
...
: Common Scoring Settings for Kickers {#tbl-scoringKickersPDF}
...
: Common Scoring Settings for Team Defense/Special Teams {#tbl-scoringDSTPDF}
...
: Common Scoring Settings for Individual Defensive Players {#tbl-scoringIDPPDF}
...
# Conclusion {#sec-fantasyFootballConclusion}

This chapter provided a brief primer on (American) football and fantasy football.
Football is the most widely watched sport in the United States.
The goal in football is for a team to score more points than their opponent.
In football, points can be scored by scoring a touchdown (6 points) followed by successfully kicking a point-after-touchdown (1 additional point) or successfully completing a two-point conversion (2 additional points), kicking a field goal (3 points), or getting a safety (2 points).

Fantasy football is an online game where participants assemble (i.e., "draft") imaginary teams composed of real-life NFL players.
The participants or "managers" make decisions such as selecting which players to draft, selecting which players start on a weekly basis, identifying players to pick up from the waiver wire, and making trades with other teams.
Fantasy football relies heavily on prediction—trying to predict which players will perform best and selecting them accordingly.
In the game of fantasy football, managers accumulate points on a weekly basis based on players' actual statistical performances in NFL games.
