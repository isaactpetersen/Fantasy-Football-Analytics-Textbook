...
Which variables are important to be included as control variables can be informed by the use of [causal diagrams](#sec-causalDiagrams), as described in @sec-causalDiagrams.
...
Confounding occurs when two variables have a spurious or noncausal association because both variables are caused by another variable(s) [@DOnofrio2020].
...
M-bias is a form of collider bias that occurs when two variables that are not causally related, `A` and `B`, both influence a collider, `C`, and each (`A` and `B`) also influences a separate variable—e.g., `A` influences `D` and `B` influences `E`.
M-bias is so-named from the M-shape of the DAG.

### Butterfly Bias {#sec-colliderBias}

Butterfly bias occurs when both confounding and M-bias are present.
Butterfly bias is so-named from the butterfly shape of the DAG.
