...
Here is a code example of when the crowd-averaged predictions are centered on the truth for a player.
...
ggplot2::ggplot(,
  ...
  #  xend = mean(predictedValues) + maeBracketingCentered_crowd,
  #  y = 0,
  #  yend = 0,
  #  linewidth = 1.5,
  #  alpha = 0.6,
  ...
  annotate(
    ...
    y = 0.0003,
    label = paste("MAE: ", maeBracketingCentered_crowd, sep = ""),
    hjust = 0) + # left-justify
  labs(
    x = "Fantasy Points",
    y = "Density",
    title = "Density Plot of Predicted Fantasy Points for a Player",
    subtitle = "Crowd-Averaged Projection (Vertical Gray Line) Vs. Truth, i.e., Actual Fantasy Points (Dashed Line)"
  ) +
  theme_classic() +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5)) # horizontal y-axis title
```

As demonstrated above, when the crowd-averaged predictions are centered on the truth, the crowd-averaged predictions are perfectly accurate and are more accurate (MAE = `{r} maeBracketingCentered_crowd`) than the accuracy of the typical individual forecaster (MAE = `{r} maeBracketingCentered_individual`) in terms of [mean absolute error](#sec-meanAbsoluteError).

Here is a code example of when the crowd-averaged predictions bracket but are not centered on the truth for a player.
...
ggplot2::ggplot(,
  ...
  annotate(
    "segment",
    x = mean(predictedValues),
    xend = mean(predictedValues) + maeBracketingNotCentered1_crowd,
    y = 0,
    yend = 0,
    linewidth = 1.5,
    alpha = 0.6,
  ...
  annotate(
    "text",
    x = mean(predictedValues) + (maeBracketingNotCentered1_crowd / 2),
    y = 0.0003,
    label = paste("MAE: ", maeBracketingNotCentered1_crowd, sep = ""),
...
As demonstrated above, when the crowd-averaged predictions bracket but are not centered on the truth, the crowd-averaged predictions (MAE = `{r} maeBracketingNotCentered1_crowd`) are more accurate than the accuracy of the typical individual forecaster (MAE = `{r} maeBracketingNotCentered1_individual`) in terms of [mean absolute error](#sec-meanAbsoluteError).

Here is another code example of when the crowd-averaged predictions bracket but are not centered on the truth for a player.
...
ggplot2::ggplot(,
  ...
  annotate(
    "segment",
    x = mean(predictedValues),
    xend = mean(predictedValues) - maeBracketingNotCentered2_crowd,
    y = 0,
    yend = 0,
    linewidth = 1.5,
    alpha = 0.6,
  ...
  annotate(
    "text",
    x = mean(predictedValues) - (maeBracketingNotCentered2_crowd / 2),
    y = 0.0003,
    label = paste("MAE: ", maeBracketingNotCentered2_crowd, sep = ""),
    hjust = 0.5) + # center-justify
  labs(
    x = "Fantasy Points",
    y = "Density",
...
As demonstrated above, when the crowd-averaged predictions bracket but are not centered on the truth, the crowd-averaged predictions (MAE = `{r} maeBracketingNotCentered2_crowd`) are more accurate than the accuracy of the typical individual forecaster (MAE = `{r} maeBracketingNotCentered2_crowd`) in terms of [mean absolute error](#sec-meanAbsoluteError), but they can still be less accurate than the individual forecaster who is at the 50th percentile in accuracy.

Here is a code example of when the crowd-averaged predictions do not bracket the truth for a player.
...
ggplot2::ggplot(,
  ...
  annotate(
    "segment",
    x = mean(predictedValues),
    xend = mean(predictedValues) + maeNoBracketing1_crowd,
    y = 0,
    yend = 0,
    linewidth = 1.5,
    alpha = 0.6,
  ...
  annotate(
    "text",
    x = mean(predictedValues) + (maeNoBracketing1_crowd / 2),
    y = 0.0003,
    label = paste("MAE: ", maeNoBracketing1_crowd, sep = ""),
    hjust = 0.5) + # center-justify
  labs(
    x = "Fantasy Points",
    y = "Density",
...
As demonstrated above, when the crowd-averaged predictions do not bracket the truth, the crowd-averaged predictions (MAE = `{r} maeNoBracketing1_crowd`) are as accurate as the accuracy of the typical individual forecaster (MAE = `{r} maeNoBracketing1_individual`) in terms of [mean absolute error](#sec-meanAbsoluteError).

Here is another code example of when the crowd-averaged predictions do not bracket the truth for a player.
The distribution of projected points versus truth is depicted in @fig-noBracketing2.

```{r}
predictedValues <- c(0,65,70,75,80,85,90,95)
actualValue <- 100
errors <- predictedValues - actualValue
...
ggplot2::ggplot(,
  ...
  annotate(
    "segment",
    x = mean(predictedValues),
    xend = mean(predictedValues) + maeNoBracketing2_crowd,
    y = 0,
    yend = 0,
    linewidth = 1.5,
    alpha = 0.6,
  ...
  annotate(
    "text",
    x = mean(predictedValues) + (maeNoBracketing2_crowd / 2),
    y = 0.0006,
    label = paste("MAE: ", maeNoBracketing2_crowd, sep = ""),
    hjust = 0.5) + # center-justify
  labs(
    x = "Fantasy Points",
    y = "Density",
...
As demonstrated above, when the crowd-averaged predictions do not bracket the truth, the crowd-averaged predictions (MAE = `{r} maeNoBracketing2_crowd`) are as accurate as the accuracy of the typical individual forecaster (MAE = `{r} maeNoBracketing2_crowd`) in terms of [mean absolute error](#sec-meanAbsoluteError), but they can be less accurate than the individual forecaster who is at the 50th percentile in accuracy.

In conclusion, as demonstrated from these examples, when the crowd-averaged predictions are centered on the truth, the crowd-averaged predictions are perfectly accurate and are more accurate than the accuracy of the typical individual forecaster in terms of [mean absolute error](#sec-meanAbsoluteError).
...
When the crowd-averaged predictions do not bracket the truth, the crowd-averaged predictions are as accurate as the accuracy of the typical individual forecaster in terms of [mean absolute error](#sec-meanAbsoluteError), but they can be less accurate than the individual forecaster who is at the 50th percentile in accuracy.
...
That is, depending on the bracketing rate, the crowd-averaged projections can still be less accurate than most individual projections.
