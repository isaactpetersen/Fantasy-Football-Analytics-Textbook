...
#### Linear Regression Model {#sec-fantasyPointsByAgeExperienceModelsRegression}

```{r}
pointsPerSeason_regressionModel <- lm(
  fantasy_points ~ 1,
  data = player_stats_seasonal_offense_subset,
  na.action = "na.exclude"
)

summary(pointsPerSeason_regressionModel)
summary(pointsPerSeason_regressionModel)$r.squared
AIC(pointsPerSeason_regressionModel)
MuMIn::AICc(pointsPerSeason_regressionModel)
```
...
```{r}
lmVsMixedModel <- list(
  "linearRegression" = pointsPerSeason_regressionModel,
...
```{r}
AIC(
  pointsPerSeason_regressionModel,
  pointsPerSeason_randomIntercepts,
  pointsPerSeason_positionAgeFixedLinearSlopes,
  pointsPerSeason_positionAgeRandomLinearSlopes,
  pointsPerSeason_positionAgeRandomLinearFixedQuadraticSlopes,
...
AICcmodavg::bictab(mixedModels) # throws error (can't mix lm with lmer)
#bbmle::AICctab(mixedModels) # throws error (different numbers of observations)
...
```{r}
deviance(pointsPerSeason_regressionModel)
...
logLik(pointsPerSeason_regressionModel)
