...
We can convert a covariance to a [correlation](#sec-correlationExamplesPearson) by standardizing it; i.e., by dividing by the multiplication of the standard deviation of $x$ and the standard deviation of $y$, as in @eq-covToCor:

$$
r_{xy} = \frac{\text{Cov}(x, y)}{\sigma_x \cdot \sigma_y}
$$ {#eq-covToCor}

For instance, for the covariance between height and fantasy points, we can convert it to a [correlation](#sec-correlationExamplesPearson) (we subset to just those rows that have data on both $x$ and $y$ to ensure we compute the standard deviations using the same values that are included in the estimate of the covariance):
