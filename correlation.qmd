...
#      newValue <- paste(
#        " = ",
#        petersenlab::suppressLeadingZero(specify_decimal(value, digits)),
#        sep = "")
#    }
#  
#    return(newValue)
#  }
#
#textCorrelation <- function(x, y, decimals = 2){
#  substitute(
#  paste(
...
#    sep = ""),
#  list(
#    valueR = noquote(sub("0.", ".", sprintf("%.2f", round(cor.test(x, y)$estimate[[1]], decimals)))),
#    valueDF = cor.test(x, y)$parameter[[1]],
#    valueP = pValue(cor.test(x, y)$p.value[[1]])
#    )
#  )
#}
#
...
#}
#
#textCorrelation(player_stats_seasonal$height, player_stats_seasonal$fantasyPoints)
```
...
We could report this finding as follows: There was a statistically significant, weak negative association between height and fantasy points ($r(`{r} pearson_df`) = `{r} noquote(petersenlab::apa(pearson_r, decimals = 2, leading = FALSE))`$, $p < .001$); the taller a player was, the fewer fantasy points they tended to score.
...
We could address this assumption by analyzing only one season from each player.
However, for simplicity, we present results from the whole dataframe.
