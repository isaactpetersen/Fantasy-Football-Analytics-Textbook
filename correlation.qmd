...
The $r$ value (`{r} pearson_r`) is slightly negative.
...
We could report this finding as follows: There was a statistically significant, weak negative association between height and fantasy points ($r[$`{r} pearson_df`$] =$ `{r} pearson_r`, $p < .001$); the taller a player was, the fewer fantasy points they tended to score.
...
In @sec-mixedModels, we discuss [mixed model](#sec-mixedModels) approaches that handle repeated measures and other data that violate assumptions of non-independence that are shared by correlation and [multiple regression](#sec-multipleRegression).
