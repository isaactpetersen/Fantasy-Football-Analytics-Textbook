...
The [Pearson correlation coefficient](Pearson Correlation), $r$, is an example of a standardized index of the covariance between two variables.
...
A scatterplot with best-fit line is in @fig-scatterplotWithBestFitLine.
...
We can estimate a distance correlation using the `correlation()` function of the correlation package [@R-correlation; @Makowski2020].
...
Here is a Pearson correlation matrix:
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `partialcor.table()` function that generates a partial correlation matrix.
Here is a partial correlation matrix, controlling for the player's age:
...
#| fig-cap: "Scatterplot With Outlier."
#| fig-alt: "Scatterplot With Outlier."
#| code-fold: true
...
Note how the association was not close to being statistically significant with the outlier, but with the outlier added, the association is statistically significant.
...
We can estimate the each using the `correlation()` function of the correlation package [@R-correlation; @Makowski2020].
...
Consider the following association between air yards share and receiving yards:

```{r}
cor.test(
  player_stats_seasonal$air_yards_share,
  player_stats_seasonal$receiving_yards
)
```

There is a statistically significant, moderate positive association between air yards share and receiving yards, as depicted in @fig-scatterplotWithoutRangeRestriction.

```{r}
#| label: fig-scatterplotWithoutRangeRestriction
#| fig-cap: "Scatterplot of Player Receiving Yards (Season) by Air Yard Share. The best-fit line is in black."
#| fig-alt: "Scatterplot of Player Receiving Yards (Season) by Air Yard Share. The best-fit line is in black."
#| code-fold: true

plot_scatterplotWithoutRangeRestriction <- ggplot2::ggplot(
  data = player_stats_seasonal %>% filter(air_yards_share > 0),
  aes(
    x = air_yards_share,
    y = receiving_yards)) +
  geom_point(
  ...
  labs(
    x = "Air Yards Share",
    y = "Receiving Yards",
    title = "Receiving Yards (Season) by Air Yards Share",
    subtitle = "(Among Players with Air Yards Share > 0)"
  ) +
  theme_classic()
...
Now, consider the same association when we restrict the range to examine only those players whose air yards share was more than 25 (thus resulting in less variability):

```{r}
cor.test(
  player_stats_seasonal %>% 
    filter(air_yards_share > 25) %>% 
    select(air_yards_share) %>% 
    pull(),
  player_stats_seasonal %>% 
    filter(air_yards_share > 25) %>% 
    select(receiving_yards) %>% 
    pull()
)
```

The association is no longer statistically significant, and the association is weak in terms of [effect size](#sec-effectSizeThresholds), as depicted in @fig-scatterplotWithRangeRestriction.

```{r}
#| label: fig-scatterplotWithRangeRestriction
#| fig-cap: "Scatterplot of Player Receiving Yards (Season) by Target Share. The linear best-fit line is in black. The nonlinear best-fit line is in blue."
#| fig-alt: "Scatterplot of Player Receiving Yards (Season) by Target Share. The linear best-fit line is in black. The nonlinear best-fit line is in blue."
#| code-fold: true

plot_scatterplotWithRangeRestriction <- ggplot2::ggplot(
  data = player_stats_seasonal %>% filter(air_yards_share > 25),
  aes(
    x = air_yards_share,
    y = receiving_yards)) +
  geom_point(
  ...
  labs(
    ...
    y = "Receiving Yards",
    title = "Receiving Yards (Season) by Air Yards Share",
    subtitle = "(Among Players with Air Yards Share > 25)"
  ) +
  theme_classic()
