...
Also note that, although the ([Pearson](#sec-correlationExamplesPearson)) correlation reflects the strength of the (linear) association between two variables, it does not reflect the (unstandardized) slope of that association, as depicted in @fig-correlationExamples.

::: {#fig-correlationExamples}
![](images/correlationExamples.png){fig-alt="Several sets of ($x$, $y$) points, with the ([Pearson](#sec-correlationExamplesPearson)) correlation coefficient of $x$ and $y$ for each set. The ([Pearson](#sec-correlationExamplesPearson)) correlation reflects the strength and direction of a linear association (top row), but not the slope of that association (middle), nor many aspects of nonlinear associations (bottom). N.B.: the figure in the center has a slope of 0 but in that case the correlation coefficient is undefined because the variance of $y$ is zero. From <https://en.wikipedia.org/wiki/File:Correlation_examples2.svg>."}

Several sets of ($x$, $y$) points, with the ([Pearson](#sec-correlationExamplesPearson)) correlation coefficient of $x$ and $y$ for each set. The ([Pearson](#sec-correlationExamplesPearson)) correlation reflects the strength and direction of a linear association (top row), but not the slope of that association (middle), nor many aspects of nonlinear associations (bottom). N.B.: the figure in the center has a slope of 0 but in that case the correlation coefficient is undefined because the variance of $y$ is zero. From <https://en.wikipedia.org/wiki/File:Correlation_examples2.svg>.
:::

The [Pearson](#sec-correlationExamplesPearson) correlation coefficient can be greatly impacted by the extent of variability in the data, differences in the shapes of the two distributions, [nonlinearity](#sec-correlationExamplesNonlinear), [outliers](#sec-correlationOutliers), characteristics of the sample, and measurement error [@Goodwin2006].
These factors tend to have greater impact when the sample size is smaller (compared to when the sample size is larger).
The [Pearson](#sec-correlationExamplesPearson) correlation coefficient tends to be smaller when:

- there is low variability in one or both variables (i.e., [restricted range](#sec-correlationRestrictedRange))
- the two distributions show dissimilar shapes (in terms of [skewness and kurtosis](#sec-descriptiveStatisticsShape))
- the underlying association is nonlinear, such that [Pearson](#sec-correlationExamplesPearson) correlation does not capture the association
- [outliers](#sec-correlationOutliers) weaken the apparent linear trend
- the association differs across subgroups of the sample (i.e., [moderation](#sec-moderation))
...
- [outliers](#sec-correlationOutliers) that happen to fall along a linear trend
- systematic measurement error (e.g., common method bias)
- third variable [confounding](#sec-causalDiagramConfounding)

## Example: Player Height and Fantasy Points {#sec-correlationHeightFantasyPoints}
...
It represents the average product of two variables' deviations from their respective means.
The formula for (sample) covariance is in @eq-covariance and is the numerator to the formula for [correlation](#sec-correlationExamplesPearson).
...
where $n$ is the sample size, $x$ is the [predictor variable](#sec-correlationalStudy), $y$ is the [outcome variable](#sec-correlationalStudy), $x_i$ is the ith observation on the [predictor variable](#sec-correlationalStudy), $y_i$ is the ith observation of the [outcome variable](#sec-correlationalStudy), $\bar{x}$ is the mean of the [predictor variable](#sec-correlationalStudy), $\bar{y}$ is the mean of the [outcome variable](#sec-correlationalStudy), $r_{xy}$ is the correlation between the [predictor](#sec-correlationalStudy) and [outcome](#sec-correlationalStudy) variables, $s_x$ is the standard deviation of the [predictor variable](#sec-correlationalStudy), and $s_y$ is the standard deviation of the [outcome variable](#sec-correlationalStudy).
...
```{r}
complete_cases <- player_stats_seasonal[complete.cases(player_stats_seasonal$height, player_stats_seasonal$fantasyPoints),]
...
\begin{aligned}
  ...
  &= s^2
\end{aligned}
$$ {#eq-covarianceToVariance}

The following syntax shows the variance-covariance matrix of a set of variables, where the covariance of a variable with itself is the variable's variance.
The variances are the values on the diagonal; the covariances are the values on the off-diagonal.
...
The covariance is unstandardized—its metric depends on the metrics of the two variables examined in the association.
However, it can be helpful to use a standardized index of the association between the variables.
A standardized index means that the index is on a common metric and does not depend on the metrics of the two variables.
This is valuable because it allows fairly comparing the strength of associations between sets of variables with different metrics, which is meaningful for estimating the [effect size](#sec-practicalSignificance)—i.e., the strength of the association.
The [Pearson correlation coefficient](#sec-correlationExamplesPearson), $r$, is an example of a standardized index of the covariance between two variables because it has a common metric (whose possible values range from −1 to +1, regardless of the variables examined in the association).
We can convert a covariance to a [correlation](#sec-correlationExamplesPearson) by standardizing it; i.e., by dividing by the multiplication of the standard deviation of $x$ and the standard deviation of $y$, as in @eq-pearsonCorrelation.

For instance, for the covariance between height and fantasy points, we can convert it to a [correlation](#sec-correlationExamplesPearson):
...
We can also convert a correlation back to covariance using the formula in @eq-covariance.

```{r}
cor_value * sd(complete_cases$height, na.rm = TRUE) * sd(complete_cases$fantasyPoints, na.rm = TRUE)
```

### Pearson Correlation {#sec-correlationExamplesPearson}

Correlation is a standardized index of the association between two variables.
Pearson correlation is the most common type of correlation.
The formula for Pearson correlation is in @eq-pearsonCorrelation.
...
You can compute a correlation manually using the [*z* scores](#sec-zScores), as in @eq-pearsonCorrelation:

```{r}
sum(scale(complete_cases$height) * scale(complete_cases$fantasyPoints)) / nrow(complete_cases)
```
...
The number of degrees of freedom ($df$) for a statistical test of a correlation coefficient is in @eq-correlationDF.
...
```{r}
cor(
  player_stats_seasonal[,c("draft_round","fantasyPoints")],
  use = "pairwise.complete.obs",
  method = "spearman")

cor.test(
  ~ draft_round + fantasyPoints,
  data = player_stats_seasonal,
  method = "spearman"
)
```
...
Below, we examine a partial [Pearson correlation](#sec-correlationExamplesPearson) between height and fantasy points after controlling for age using the the `psych::partial.r()` function from the `psych` package [@R-psych].
...
Examples of a nonlinear correlation include the correlation ratio [$\eta$; @Goodwin2006], distance correlation, and the xi ($\xi$) coefficient [@Chatterjee2021].
...
# the correlation ratio (eta) is equivalent to square root of R-squared (when the predictor is converted to a factor)
...
Rvalue <- sqrt(Rsquared)
Rvalue
```

We can estimate a distance correlation using the `correlation::correlation()` function of the correlation package [@R-correlation; @Makowski2020_packages].
...
correlation::correlation(
  data = player_stats_seasonal %>% 
    ...
    select(draft_round, fantasyPoints),
  method = "distance"
)
```

A nonlinear correlation can be estimated using the xi ($\xi$) coefficient from the `XICOR::xicor()` function of the `XICOR` package [@R-XICOR; @Chatterjee2021]:

```{r}
#| label: xi-coefficient
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::cor.table()` function that generates a correlation matrix of variables, including the $r$ value, number of observations ($n$), asterisks denoting statistical significance, and $p$-value for each association.
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::partialcor.table()` function that generates a [partial correlation](#sec-correlationExamplesPartial) matrix.
...
We can depict a correlation matrix plot (correlogram) using the `GGally::ggcorr()` function of the `GGally` [@R-GGally] package, as shown in @fig-correlogram.

```{r}
#| label: fig-correlogram
#| fig-cap: "Correlation Matrix Plot (Correlogram)."
#| fig-alt: "Correlation Matrix Plot (Correlogram)."
...
In @sec-multipleRegressionNonIndependence, we demonstrate how to account for non-independence of observations using cluster-robust standard errors.
...
Outliers can arise from errors that arise from data collection, measurement, or data entry, or they could represent valid (albeit atypical) values [@Goodwin2006].
Outliers can either artificially weaken the estimate of association—if they weaken the apparent linear trend—or artificially strengthen it—if they happen to fall along a linear trend.
...
We can estimate the each using the `correlation::correlation()` function of the correlation package [@R-correlation; @Makowski2020_packages].
...
Correlation is a standardized index of the association between variables.
...
The correlation coefficient can be heavily impacted by [outliers](#sec-correlationOutliers) and [restricted range](#sec-correlationRestrictedRange).
