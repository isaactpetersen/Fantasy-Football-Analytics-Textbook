...
As the maxim goes, "correlation does not mean causation"—just because two variables are associated does not necessarily mean that they are causally related.
There could be third variables (i.e., confounds) that act as a common cause of both the predictor and outcome variables, thus explaining why they are correlated.
...
Or, the variables could be associated due to the opposite direction of effect (i.e., the outcome variable may influence the predictor variable).
...
For example, if the effect of consuming sports drinks on player performance differs for Quarterbacks and Wide Receivers, the interaction could be depicted below:

![Moderation Example](images/moderationExample1.png)

![Moderation Example](images/moderationExample2.png)
