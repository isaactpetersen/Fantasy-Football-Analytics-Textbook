...
::: {#fig-confusionMatrix}
![](images/confusionMatrix.png){fig-alt="A Two-by-Two Confusion Matrix. From @Petersen2024a and @PetersenPrinciplesPsychAssessment."}
...
![](images/confusionMatrix1.png){fig-alt="Example of a Two-by-Two Confusion Matrix. TP = true positives; TN = true negatives; FP = false positives; FN = false negatives. From @Petersen2024a and @PetersenPrinciplesPsychAssessment."}
...
![](images/confusionMatrix2.png){fig-alt="Example of a Two-by-Two Confusion Matrix With Marginal Sums. TP = true positives; TN = true negatives; FP = false positives; FN = false negatives. From @Petersen2024a and @PetersenPrinciplesPsychAssessment."}
...
![](images/confusionMatrix3.png){fig-alt="Example of a Two-by-Two Confusion Matrix With Marginal Sums and Marginal Probabilities. TP = true positives; TN = true negatives; FP = false positives; FN = false negatives; BR = base rate; SR = selection ratio. From @Petersen2024a and @PetersenPrinciplesPsychAssessment."}
...
![](images/confusionMatrix4.png){fig-alt="Chance Expectancies in Two-by-Two Confusion Matrix. BR = base rate; SR = selection ratio. From @Petersen2024a and @PetersenPrinciplesPsychAssessment."}
...
### Set a Cutoff/Threshold {#sec-setCutoff}

For evaluating sensitivity, specificity, positive predictive value, negative predictive value, and the [receiver operating characteristic (ROC) curve](#sec-roc), we need to make a dichotomous decision/prediction.
For this example, our goal is to predict which Running Backs or Wide Receivers will score 300 or more fantasy points.
We will predict a player as scoring 300 or more fantasy points for a given projection source if that projection source projected the player to score 300 or more fantasy points.
Thus, our cutoff or threshold is 300 or more projected points.
The same player has multiple rows in the data file—one for each projection source.
...
table(projectionsWithActuals_seasonal_subset$truthFactor)
```
...
@fig-aucExampleEmpirical depicts an empirical ROC plot where lines are drawn to connect the hit and false alarm rates, in predicting Running Backs and Wide Receivers who score 300 or more fantasy points.

```{r}
#| label: fig-aucExampleEmpirical
#| fig-cap: "Empirical Receiver Operating Characteristic (ROC) Curve for Predicting Running Backs and Wide Receivers that Score 300 or More Fantasy Points."
#| fig-alt: "Empirical Receiver Operating Characteristic (ROC) Curve for Predicting Running Backs and Wide Receivers that Score 300 or More Fantasy Points."
#| fig-width: 8
#| fig-height: 8
...
@fig-aucExample depicts an ROC curve where a smoothed and fitted curve is drawn to connect the hit and false alarm rates, in predicting Running Backs and Wide Receivers who score 300 or more fantasy points.

```{r}
#| label: fig-aucExample
#| fig-cap: "Smoothed Receiver Operating Characteristic (ROC) Curve for Predicting Running Backs and Wide Receivers that Score 300 or More Fantasy Points."
#| fig-alt: "Smoothed Receiver Operating Characteristic (ROC) Curve for Predicting Running Backs and Wide Receivers that Score 300 or More Fantasy Points."
#| fig-width: 8
#| fig-height: 8
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::accuracyAtCutoff()` function that computes many accuracy indices for the prediction of categorical outcome variables.
In the example below, we compute the accuracy of predictions for which Running Backs or Wide Receivers will score 300 or more fantasy points, using a cutoff for projected fantasy points of 300.
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::accuracyAtEachCutoff()` function that computes many accuracy indices for the prediction of categorical outcome variables at each possible cutoff.
In the example below, we compute the accuracy of predictions for which Running Backs or Wide Receivers will score 300 or more fantasy points, at each possible cutoff for projected fantasy points.
...
As described in @sec-accuracyCutoff, as the cutoff increases (becomes more conservative), sensitivity decreases.
...
As described in @sec-accuracyCutoff, as the cutoff increases (becomes more conservative), specificity increases.
...
As described in @sec-accuracyCutoff, as the [base rate](#sec-baseRate) increases, PPV increases.
As the [base rate](#sec-baseRate) decreases, PPV decreases.
PPV also differs as a function of the cutoff.
As described in @sec-accuracyCutoff, as the cutoff increases (becomes more conservative), PPV increases.
...
As described in @sec-accuracyCutoff, as the [base rate](#sec-baseRate) increases, NPV decreases.
As the [base rate](#sec-baseRate) decreases, NPV increases.
NPV also differs as a function of the cutoff.
As described in @sec-accuracyCutoff, as the cutoff increases (becomes more conservative), NPV decreases.
...
### Optimal Cutoff {#sec-optimalCutoff}

The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::optimalCutoff()` function that identifies the cutoff that optimizes each of various accuracy estimates.

```{r}
#| eval: false

petersenlab::optimalCutoff(
  predicted = projectionsWithActuals_seasonal_subset$raw_points,
  actual = projectionsWithActuals_seasonal_subset$truth)
```
...
optimalCutoff <- petersenlab::optimalCutoff(
  predicted = projectionsWithActuals_seasonal_subset$raw_points,
  actual = projectionsWithActuals_seasonal_subset$truth)
```
...
for(i in 1:length(optimalCutoff)){
  print(head(optimalCutoff[[i]]))
}
```
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::accuracyOverall()` function that computes many threshold-independent accuracy indices.
...
The `petersenlab::accuracyOverall()` function of the [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] provides the option in the function to drop undefined values so you can still generate an estimate of accuracy despite undefined values.
...
For example, here is an example of estimating predictive $R^2$ using [10-fold cross-validation](#sec-kFoldCrossValidation) using the `tidymodels` ecosystem of packages [@R-tidymodels; @Kuhn2020_packages].
...
We can obtain standardized regression coefficients by standardizing the predictors and outcome using the `base::scale()` function in `R`.
...
Gold-standard recommendations include examining the predicted values in relation to the observed values using locally estimated scatterplot smoothing (LOESS) [@Austin2014], such as in @fig-calibrationPlotSeasonalOverall.
...
- [discrimination](#sec-discrimination):
  - [standardized regression coefficient (beta)](#sec-standardizedRegressionCoefficient)
- [calibration](#sec-calibration):
  - [calibration plot](#sec-calibrationPlot)
...
FantasyFootballAnalytics.net also provides tools to evaluate the accuracy of historical projections [@FFA-accuracy]: <https://fantasyfootballanalytics.net/ffanalytics-web-app-accuracy-tab>.
For instance, the web application allows evaluating the accuracy of historical projections:
...
accuracyIndices_seasonal_overall <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal$raw_points,
  actual = projectionsWithActuals_seasonal$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_overall <- round(accuracyIndices_seasonal_overall$rsquared, digits = 2)
me_seasonal_overall <- round(accuracyIndices_seasonal_overall$ME, digits = 2)
mae_seasonal_overall <- round(accuracyIndices_seasonal_overall$MAE, digits = 2)
```

In the data object, each projection source has a row for each player and season.
Thus, the $R^2$ may be higher for this aggregated object than it is for many of the individual sources.
To calculate the accuracy by source, we could use the following code:

```{r}
accuracy_by_source <- projectionsWithActuals_seasonal |>
...
A calibration plot of the seasonal projections for all players (across all positions) is in @fig-calibrationPlotSeasonalOverall.
The best-fit LOESS line is the yellow line.
The intercept from the best-fit line is positive; the slope is less than one.
The predicted values are less than the observed values at lower levels of projected points, whereas the predicted values are higher than the observed values at higher levels of projected points, consistent with the overextremity form of [miscalibration](#sec-calibration).
...
We calculate the 95% prediction interval based on a LOESS model, which allows nonlinear associations, using the `msir::loess.sd()` function of the `msir` package [@R-msir; @Scrucca2011_packages].
...
In this case, based on the intercept and slope of the [calibration plot](#sec-calibrationPlot) in @fig-calibrationPlotSeasonalOverall, the 95% confidence interval of the observed value does not include the reference line (i.e., the actual observed value) at lower or upper levels of the predicted values, so the predictions are [miscalibrated](#sec-calibration) at lower and upper levels of the predicted values.
...
accuracyIndices_seasonal_overall_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_overall_subset <- round(accuracyIndices_seasonal_overall_subset$rsquared, digits = 2)
me_seasonal_overall_subset <- round(accuracyIndices_seasonal_overall_subset$ME, digits = 2)
mae_seasonal_overall_subset <- round(accuracyIndices_seasonal_overall_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_overall_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_overall_crowd <- round(accuracyIndices_seasonal_overall_crowd$rsquared, digits = 2)
me_seasonal_overall_crowd <- round(accuracyIndices_seasonal_overall_crowd$ME, digits = 2)
mae_seasonal_overall_crowd <- round(accuracyIndices_seasonal_overall_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_overall_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal$points[which(crowdAveragedProjectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_overall_crowd_subset <- round(accuracyIndices_seasonal_overall_crowd_subset$rsquared, digits = 2)
me_seasonal_overall_crowd_subset <- round(accuracyIndices_seasonal_overall_crowd_subset$ME, digits = 2)
mae_seasonal_overall_crowd_subset <- round(accuracyIndices_seasonal_overall_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_qb <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_qb$raw_points,
  actual = projectionsWithActuals_seasonal_qb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_qb <- round(accuracyIndices_seasonal_qb$rsquared, digits = 2)
me_seasonal_qb <- round(accuracyIndices_seasonal_qb$ME, digits = 2)
mae_seasonal_qb <- round(accuracyIndices_seasonal_qb$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Quarterbacks is in @fig-calibrationPlotSeasonalQBs.
...
accuracyIndices_seasonal_qb_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_qb$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_qb$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_qb_subset <- round(accuracyIndices_seasonal_qb_subset$rsquared, digits = 2)
me_seasonal_qb_subset <- round(accuracyIndices_seasonal_qb_subset$ME, digits = 2)
mae_seasonal_qb_subset <- round(accuracyIndices_seasonal_qb_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_qb_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_qb$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_qb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_qb_crowd <- round(accuracyIndices_seasonal_qb_crowd$rsquared, digits = 2)
me_seasonal_qb_crowd <- round(accuracyIndices_seasonal_qb_crowd$ME, digits = 2)
mae_seasonal_qb_crowd <- round(accuracyIndices_seasonal_qb_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_qb_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_qb$points[which(crowdAveragedProjectionsWithActuals_seasonal_qb$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_qb$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_qb$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_qb_crowd_subset <- round(accuracyIndices_seasonal_qb_crowd_subset$rsquared, digits = 2)
me_seasonal_qb_crowd_subset <- round(accuracyIndices_seasonal_qb_crowd_subset$ME, digits = 2)
mae_seasonal_qb_crowd_subset <- round(accuracyIndices_seasonal_qb_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_rb <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_rb$raw_points,
  actual = projectionsWithActuals_seasonal_rb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_rb <- round(accuracyIndices_seasonal_rb$rsquared, digits = 2)
me_seasonal_rb <- round(accuracyIndices_seasonal_rb$ME, digits = 2)
mae_seasonal_rb <- round(accuracyIndices_seasonal_rb$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Running Backs is in @fig-calibrationPlotSeasonalRBs.
...
accuracyIndices_seasonal_rb_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_rb$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_rb$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_rb_subset <- round(accuracyIndices_seasonal_rb_subset$rsquared, digits = 2)
me_seasonal_rb_subset <- round(accuracyIndices_seasonal_rb_subset$ME, digits = 2)
mae_seasonal_rb_subset <- round(accuracyIndices_seasonal_rb_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_rb_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_rb$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_rb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_rb_crowd <- round(accuracyIndices_seasonal_rb_crowd$rsquared, digits = 2)
me_seasonal_rb_crowd <- round(accuracyIndices_seasonal_rb_crowd$ME, digits = 2)
mae_seasonal_rb_crowd <- round(accuracyIndices_seasonal_rb_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_rb_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_rb$points[which(crowdAveragedProjectionsWithActuals_seasonal_rb$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_rb$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_rb$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_rb_crowd_subset <- round(accuracyIndices_seasonal_rb_crowd_subset$rsquared, digits = 2)
me_seasonal_rb_crowd_subset <- round(accuracyIndices_seasonal_rb_crowd_subset$ME, digits = 2)
mae_seasonal_rb_crowd_subset <- round(accuracyIndices_seasonal_rb_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_wr <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_wr$raw_points,
  actual = projectionsWithActuals_seasonal_wr$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_wr <- round(accuracyIndices_seasonal_wr$rsquared, digits = 2)
me_seasonal_wr <- round(accuracyIndices_seasonal_wr$ME, digits = 2)
mae_seasonal_wr <- round(accuracyIndices_seasonal_wr$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Wide Receivers is in @fig-calibrationPlotSeasonalWRs.
...
accuracyIndices_seasonal_wr_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_wr$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_wr$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_wr_subset <- round(accuracyIndices_seasonal_wr_subset$rsquared, digits = 2)
me_seasonal_wr_subset <- round(accuracyIndices_seasonal_wr_subset$ME, digits = 2)
mae_seasonal_wr_subset <- round(accuracyIndices_seasonal_wr_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_wr_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_wr$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_wr$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_wr_crowd <- round(accuracyIndices_seasonal_wr_crowd$rsquared, digits = 2)
me_seasonal_wr_crowd <- round(accuracyIndices_seasonal_wr_crowd$ME, digits = 2)
mae_seasonal_wr_crowd <- round(accuracyIndices_seasonal_wr_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_wr_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_wr$points[which(crowdAveragedProjectionsWithActuals_seasonal_wr$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_wr$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_wr$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_wr_crowd_subset <- round(accuracyIndices_seasonal_wr_crowd_subset$rsquared, digits = 2)
me_seasonal_wr_crowd_subset <- round(accuracyIndices_seasonal_wr_crowd_subset$ME, digits = 2)
mae_seasonal_wr_crowd_subset <- round(accuracyIndices_seasonal_wr_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_te <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_te$raw_points,
  actual = projectionsWithActuals_seasonal_te$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_te <- round(accuracyIndices_seasonal_te$rsquared, digits = 2)
me_seasonal_te <- round(accuracyIndices_seasonal_te$ME, digits = 2)
mae_seasonal_te <- round(accuracyIndices_seasonal_te$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Tight Ends is in @fig-calibrationPlotSeasonalTEs.
...
accuracyIndices_seasonal_te_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_te$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_te$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_te_subset <- round(accuracyIndices_seasonal_te_subset$rsquared, digits = 2)
me_seasonal_te_subset <- round(accuracyIndices_seasonal_te_subset$ME, digits = 2)
mae_seasonal_te_subset <- round(accuracyIndices_seasonal_te_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_te_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_te$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_te$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_te_crowd <- round(accuracyIndices_seasonal_te_crowd$rsquared, digits = 2)
me_seasonal_te_crowd <- round(accuracyIndices_seasonal_te_crowd$ME, digits = 2)
mae_seasonal_te_crowd <- round(accuracyIndices_seasonal_te_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_te_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_te$points[which(crowdAveragedProjectionsWithActuals_seasonal_te$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_te$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_te$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_te_crowd_subset <- round(accuracyIndices_seasonal_te_crowd_subset$rsquared, digits = 2)
me_seasonal_te_crowd_subset <- round(accuracyIndices_seasonal_te_crowd_subset$ME, digits = 2)
mae_seasonal_te_crowd_subset <- round(accuracyIndices_seasonal_te_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_k <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_k$raw_points,
  actual = projectionsWithActuals_seasonal_k$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_k <- round(accuracyIndices_seasonal_k$rsquared, digits = 2)
me_seasonal_k <- round(accuracyIndices_seasonal_k$ME, digits = 2)
mae_seasonal_k <- round(accuracyIndices_seasonal_k$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Kickers is in @fig-calibrationPlotSeasonalKs.
...
accuracyIndices_seasonal_k_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_k$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_k$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_k_subset <- round(accuracyIndices_seasonal_k_subset$rsquared, digits = 2)
me_seasonal_k_subset <- round(accuracyIndices_seasonal_k_subset$ME, digits = 2)
mae_seasonal_k_subset <- round(accuracyIndices_seasonal_k_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_k_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_k$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_k$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_k_crowd <- round(accuracyIndices_seasonal_k_crowd$rsquared, digits = 2)
me_seasonal_k_crowd <- round(accuracyIndices_seasonal_k_crowd$ME, digits = 2)
mae_seasonal_k_crowd <- round(accuracyIndices_seasonal_k_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_k_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_k$points[which(crowdAveragedProjectionsWithActuals_seasonal_k$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_k$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_k$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_k_crowd_subset <- round(accuracyIndices_seasonal_k_crowd_subset$rsquared, digits = 2)
me_seasonal_k_crowd_subset <- round(accuracyIndices_seasonal_k_crowd_subset$ME, digits = 2)
mae_seasonal_k_crowd_subset <- round(accuracyIndices_seasonal_k_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_dl <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_dl$raw_points,
  actual = projectionsWithActuals_seasonal_dl$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_dl <- round(accuracyIndices_seasonal_dl$rsquared, digits = 2)
me_seasonal_dl <- round(accuracyIndices_seasonal_dl$ME, digits = 2)
mae_seasonal_dl <- round(accuracyIndices_seasonal_dl$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Defensive Linemen is in @fig-calibrationPlotSeasonalDLs.
...
accuracyIndices_seasonal_dl_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_dl$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_dl$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_dl_subset <- round(accuracyIndices_seasonal_dl_subset$rsquared, digits = 2)
me_seasonal_dl_subset <- round(accuracyIndices_seasonal_dl_subset$ME, digits = 2)
mae_seasonal_dl_subset <- round(accuracyIndices_seasonal_dl_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_dl_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_dl$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_dl$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_dl_crowd <- round(accuracyIndices_seasonal_dl_crowd$rsquared, digits = 2)
me_seasonal_dl_crowd <- round(accuracyIndices_seasonal_dl_crowd$ME, digits = 2)
mae_seasonal_dl_crowd <- round(accuracyIndices_seasonal_dl_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_dl_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_dl$points[which(crowdAveragedProjectionsWithActuals_seasonal_dl$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_dl$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_dl$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_dl_crowd_subset <- round(accuracyIndices_seasonal_dl_crowd_subset$rsquared, digits = 2)
me_seasonal_dl_crowd_subset <- round(accuracyIndices_seasonal_dl_crowd_subset$ME, digits = 2)
mae_seasonal_dl_crowd_subset <- round(accuracyIndices_seasonal_dl_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_lb <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_lb$raw_points,
  actual = projectionsWithActuals_seasonal_lb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_lb <- round(accuracyIndices_seasonal_lb$rsquared, digits = 2)
me_seasonal_lb <- round(accuracyIndices_seasonal_lb$ME, digits = 2)
mae_seasonal_lb <- round(accuracyIndices_seasonal_lb$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Linebackers is in @fig-calibrationPlotSeasonalLBs.
...
accuracyIndices_seasonal_lb_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_lb$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_lb$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_lb_subset <- round(accuracyIndices_seasonal_lb_subset$rsquared, digits = 2)
me_seasonal_lb_subset <- round(accuracyIndices_seasonal_lb_subset$ME, digits = 2)
mae_seasonal_lb_subset <- round(accuracyIndices_seasonal_lb_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_lb_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_lb$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_lb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_lb_crowd <- round(accuracyIndices_seasonal_lb_crowd$rsquared, digits = 2)
me_seasonal_lb_crowd <- round(accuracyIndices_seasonal_lb_crowd$ME, digits = 2)
mae_seasonal_lb_crowd <- round(accuracyIndices_seasonal_lb_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_lb_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_lb$points[which(crowdAveragedProjectionsWithActuals_seasonal_lb$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_lb$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_lb$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_lb_crowd_subset <- round(accuracyIndices_seasonal_lb_crowd_subset$rsquared, digits = 2)
me_seasonal_lb_crowd_subset <- round(accuracyIndices_seasonal_lb_crowd_subset$ME, digits = 2)
mae_seasonal_lb_crowd_subset <- round(accuracyIndices_seasonal_lb_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_db <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_db$raw_points,
  actual = projectionsWithActuals_seasonal_db$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_db <- round(accuracyIndices_seasonal_db$rsquared, digits = 2)
me_seasonal_db <- round(accuracyIndices_seasonal_db$ME, digits = 2)
mae_seasonal_db <- round(accuracyIndices_seasonal_db$MAE, digits = 2)
```

A calibration plot of the seasonal projections for Defensive backs is in @fig-calibrationPlotSeasonalDBs.
...
accuracyIndices_seasonal_db_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_seasonal_db$raw_points[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = projectionsWithActuals_seasonal_db$fantasyPoints[which(projectionsWithActuals_seasonal$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_db_subset <- round(accuracyIndices_seasonal_db_subset$rsquared, digits = 2)
me_seasonal_db_subset <- round(accuracyIndices_seasonal_db_subset$ME, digits = 2)
mae_seasonal_db_subset <- round(accuracyIndices_seasonal_db_subset$MAE, digits = 2)
```
...
accuracyIndices_seasonal_db_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_db$points,
  actual = crowdAveragedProjectionsWithActuals_seasonal_db$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_seasonal_db_crowd <- round(accuracyIndices_seasonal_db_crowd$rsquared, digits = 2)
me_seasonal_db_crowd <- round(accuracyIndices_seasonal_db_crowd$ME, digits = 2)
mae_seasonal_db_crowd <- round(accuracyIndices_seasonal_db_crowd$MAE, digits = 2)
```
...
accuracyIndices_seasonal_db_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_seasonal_db$points[which(crowdAveragedProjectionsWithActuals_seasonal_db$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  actual = crowdAveragedProjectionsWithActuals_seasonal_db$fantasyPoints[which(crowdAveragedProjectionsWithActuals_seasonal_db$player_id_season %in% playersWithHighProjectedOrActualPoints)],
  dropUndefined = TRUE
)

rsquare_seasonal_db_crowd_subset <- round(accuracyIndices_seasonal_db_crowd_subset$rsquared, digits = 2)
me_seasonal_db_crowd_subset <- round(accuracyIndices_seasonal_db_crowd_subset$ME, digits = 2)
mae_seasonal_db_crowd_subset <- round(accuracyIndices_seasonal_db_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_overall <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly$raw_points,
  actual = projectionsWithActuals_weekly$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_overall <- round(accuracyIndices_weekly_overall$rsquared, digits = 2)
me_weekly_overall <- round(accuracyIndices_weekly_overall$ME, digits = 2)
mae_weekly_overall <- round(accuracyIndices_weekly_overall$MAE, digits = 2)
```

A calibration plot of the weekly projections for all players (across all positions) is in @fig-calibrationPlotWeeklyOverall.
...
accuracyIndices_weekly_overall_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_overall_subset <- round(accuracyIndices_weekly_overall_subset$rsquared, digits = 2)
me_weekly_overall_subset <- round(accuracyIndices_weekly_overall_subset$ME, digits = 2)
mae_weekly_overall_subset <- round(accuracyIndices_weekly_overall_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_overall_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly$points,
  actual = crowdAveragedProjectionsWithActuals_weekly$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_overall_crowd <- round(accuracyIndices_weekly_overall_crowd$rsquared, digits = 2)
me_weekly_overall_crowd <- round(accuracyIndices_weekly_overall_crowd$ME, digits = 2)
mae_weekly_overall_crowd <- round(accuracyIndices_weekly_overall_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_overall_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly$points[which(crowdAveragedProjectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_overall_crowd_subset <- round(accuracyIndices_weekly_overall_crowd_subset$rsquared, digits = 2)
me_weekly_overall_crowd_subset <- round(accuracyIndices_weekly_overall_crowd_subset$ME, digits = 2)
mae_weekly_overall_crowd_subset <- round(accuracyIndices_weekly_overall_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_qb <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_qb$raw_points,
  actual = projectionsWithActuals_weekly_qb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_qb <- round(accuracyIndices_weekly_qb$rsquared, digits = 2)
me_weekly_qb <- round(accuracyIndices_weekly_qb$ME, digits = 2)
mae_weekly_qb <- round(accuracyIndices_weekly_qb$MAE, digits = 2)
```

A calibration plot of the weekly projections for Quarterbacks is in @fig-calibrationPlotWeeklyQBs.
...
accuracyIndices_weekly_qb_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_qb$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_qb$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_qb_subset <- round(accuracyIndices_weekly_qb_subset$rsquared, digits = 2)
me_weekly_qb_subset <- round(accuracyIndices_weekly_qb_subset$ME, digits = 2)
mae_weekly_qb_subset <- round(accuracyIndices_weekly_qb_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_qb_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_qb$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_qb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_qb_crowd <- round(accuracyIndices_weekly_qb_crowd$rsquared, digits = 2)
me_weekly_qb_crowd <- round(accuracyIndices_weekly_qb_crowd$ME, digits = 2)
mae_weekly_qb_crowd <- round(accuracyIndices_weekly_qb_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_qb_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_qb$points[which(crowdAveragedProjectionsWithActuals_weekly_qb$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_qb$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_qb$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_qb_crowd_subset <- round(accuracyIndices_weekly_qb_crowd_subset$rsquared, digits = 2)
me_weekly_qb_crowd_subset <- round(accuracyIndices_weekly_qb_crowd_subset$ME, digits = 2)
mae_weekly_qb_crowd_subset <- round(accuracyIndices_weekly_qb_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_rb <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_rb$raw_points,
  actual = projectionsWithActuals_weekly_rb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_rb <- round(accuracyIndices_weekly_rb$rsquared, digits = 2)
me_weekly_rb <- round(accuracyIndices_weekly_rb$ME, digits = 2)
mae_weekly_rb <- round(accuracyIndices_weekly_rb$MAE, digits = 2)
```

A calibration plot of the weekly projections for Running Backs is in @fig-calibrationPlotWeeklyRBs.
...
accuracyIndices_weekly_rb_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_rb$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_rb$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_rb_subset <- round(accuracyIndices_weekly_rb_subset$rsquared, digits = 2)
me_weekly_rb_subset <- round(accuracyIndices_weekly_rb_subset$ME, digits = 2)
mae_weekly_rb_subset <- round(accuracyIndices_weekly_rb_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_rb_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_rb$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_rb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_rb_crowd <- round(accuracyIndices_weekly_rb_crowd$rsquared, digits = 2)
me_weekly_rb_crowd <- round(accuracyIndices_weekly_rb_crowd$ME, digits = 2)
mae_weekly_rb_crowd <- round(accuracyIndices_weekly_rb_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_rb_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_rb$points[which(crowdAveragedProjectionsWithActuals_weekly_rb$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_rb$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_rb$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_rb_crowd_subset <- round(accuracyIndices_weekly_rb_crowd_subset$rsquared, digits = 2)
me_weekly_rb_crowd_subset <- round(accuracyIndices_weekly_rb_crowd_subset$ME, digits = 2)
mae_weekly_rb_crowd_subset <- round(accuracyIndices_weekly_rb_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_wr <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_wr$raw_points,
  actual = projectionsWithActuals_weekly_wr$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_wr <- round(accuracyIndices_weekly_wr$rsquared, digits = 2)
me_weekly_wr <- round(accuracyIndices_weekly_wr$ME, digits = 2)
mae_weekly_wr <- round(accuracyIndices_weekly_wr$MAE, digits = 2)
```

A calibration plot of the weekly projections for Wide Receivers is in @fig-calibrationPlotWeeklyWRs.
...
accuracyIndices_weekly_wr_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_wr$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_wr$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_wr_subset <- round(accuracyIndices_weekly_wr_subset$rsquared, digits = 2)
me_weekly_wr_subset <- round(accuracyIndices_weekly_wr_subset$ME, digits = 2)
mae_weekly_wr_subset <- round(accuracyIndices_weekly_wr_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_wr_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_wr$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_wr$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_wr_crowd <- round(accuracyIndices_weekly_wr_crowd$rsquared, digits = 2)
me_weekly_wr_crowd <- round(accuracyIndices_weekly_wr_crowd$ME, digits = 2)
mae_weekly_wr_crowd <- round(accuracyIndices_weekly_wr_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_wr_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_wr$points[which(crowdAveragedProjectionsWithActuals_weekly_wr$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_wr$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_wr$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_wr_crowd_subset <- round(accuracyIndices_weekly_wr_crowd_subset$rsquared, digits = 2)
me_weekly_wr_crowd_subset <- round(accuracyIndices_weekly_wr_crowd_subset$ME, digits = 2)
mae_weekly_wr_crowd_subset <- round(accuracyIndices_weekly_wr_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_te <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_te$raw_points,
  actual = projectionsWithActuals_weekly_te$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_te <- round(accuracyIndices_weekly_te$rsquared, digits = 2)
me_weekly_te <- round(accuracyIndices_weekly_te$ME, digits = 2)
mae_weekly_te <- round(accuracyIndices_weekly_te$MAE, digits = 2)
```

A calibration plot of the weekly projections for Tight Ends is in @fig-calibrationPlotWeeklyTEs.
...
accuracyIndices_weekly_te_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_te$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_te$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_te_subset <- round(accuracyIndices_weekly_te_subset$rsquared, digits = 2)
me_weekly_te_subset <- round(accuracyIndices_weekly_te_subset$ME, digits = 2)
mae_weekly_te_subset <- round(accuracyIndices_weekly_te_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_te_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_te$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_te$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_te_crowd <- round(accuracyIndices_weekly_te_crowd$rsquared, digits = 2)
me_weekly_te_crowd <- round(accuracyIndices_weekly_te_crowd$ME, digits = 2)
mae_weekly_te_crowd <- round(accuracyIndices_weekly_te_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_te_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_te$points[which(crowdAveragedProjectionsWithActuals_weekly_te$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_te$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_te$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_te_crowd_subset <- round(accuracyIndices_weekly_te_crowd_subset$rsquared, digits = 2)
me_weekly_te_crowd_subset <- round(accuracyIndices_weekly_te_crowd_subset$ME, digits = 2)
mae_weekly_te_crowd_subset <- round(accuracyIndices_weekly_te_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_k <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_k$raw_points,
  actual = projectionsWithActuals_weekly_k$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_k <- round(accuracyIndices_weekly_k$rsquared, digits = 2)
me_weekly_k <- round(accuracyIndices_weekly_k$ME, digits = 2)
mae_weekly_k <- round(accuracyIndices_weekly_k$MAE, digits = 2)
```

A calibration plot of the weekly projections for Kickers is in @fig-calibrationPlotWeeklyKs.
...
accuracyIndices_weekly_k_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_k$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_k$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_k_subset <- round(accuracyIndices_weekly_k_subset$rsquared, digits = 2)
me_weekly_k_subset <- round(accuracyIndices_weekly_k_subset$ME, digits = 2)
mae_weekly_k_subset <- round(accuracyIndices_weekly_k_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_k_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_k$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_k$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_k_crowd <- round(accuracyIndices_weekly_k_crowd$rsquared, digits = 2)
me_weekly_k_crowd <- round(accuracyIndices_weekly_k_crowd$ME, digits = 2)
mae_weekly_k_crowd <- round(accuracyIndices_weekly_k_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_k_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_k$points[which(crowdAveragedProjectionsWithActuals_weekly_k$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_k$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_k$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_k_crowd_subset <- round(accuracyIndices_weekly_k_crowd_subset$rsquared, digits = 2)
me_weekly_k_crowd_subset <- round(accuracyIndices_weekly_k_crowd_subset$ME, digits = 2)
mae_weekly_k_crowd_subset <- round(accuracyIndices_weekly_k_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_dl <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_dl$raw_points,
  actual = projectionsWithActuals_weekly_dl$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_dl <- round(accuracyIndices_weekly_dl$rsquared, digits = 2)
me_weekly_dl <- round(accuracyIndices_weekly_dl$ME, digits = 2)
mae_weekly_dl <- round(accuracyIndices_weekly_dl$MAE, digits = 2)
```

A calibration plot of the weekly projections for Defensive Linemen is in @fig-calibrationPlotWeeklyDLs.
...
accuracyIndices_weekly_dl_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_dl$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_dl$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_dl_subset <- round(accuracyIndices_weekly_dl_subset$rsquared, digits = 2)
me_weekly_dl_subset <- round(accuracyIndices_weekly_dl_subset$ME, digits = 2)
mae_weekly_dl_subset <- round(accuracyIndices_weekly_dl_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_dl_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_dl$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_dl$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_dl_crowd <- round(accuracyIndices_weekly_dl_crowd$rsquared, digits = 2)
me_weekly_dl_crowd <- round(accuracyIndices_weekly_dl_crowd$ME, digits = 2)
mae_weekly_dl_crowd <- round(accuracyIndices_weekly_dl_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_dl_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_dl$points[which(crowdAveragedProjectionsWithActuals_weekly_dl$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_dl$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_dl$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_dl_crowd_subset <- round(accuracyIndices_weekly_dl_crowd_subset$rsquared, digits = 2)
me_weekly_dl_crowd_subset <- round(accuracyIndices_weekly_dl_crowd_subset$ME, digits = 2)
mae_weekly_dl_crowd_subset <- round(accuracyIndices_weekly_dl_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_lb <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_lb$raw_points,
  actual = projectionsWithActuals_weekly_lb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_lb <- round(accuracyIndices_weekly_lb$rsquared, digits = 2)
me_weekly_lb <- round(accuracyIndices_weekly_lb$ME, digits = 2)
mae_weekly_lb <- round(accuracyIndices_weekly_lb$MAE, digits = 2)
```

A calibration plot of the weekly projections for Linebackers is in @fig-calibrationPlotWeeklyLBs.
...
accuracyIndices_weekly_lb_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_lb$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_lb$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_lb_subset <- round(accuracyIndices_weekly_lb_subset$rsquared, digits = 2)
me_weekly_lb_subset <- round(accuracyIndices_weekly_lb_subset$ME, digits = 2)
mae_weekly_lb_subset <- round(accuracyIndices_weekly_lb_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_lb_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_lb$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_lb$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_lb_crowd <- round(accuracyIndices_weekly_lb_crowd$rsquared, digits = 2)
me_weekly_lb_crowd <- round(accuracyIndices_weekly_lb_crowd$ME, digits = 2)
mae_weekly_lb_crowd <- round(accuracyIndices_weekly_lb_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_lb_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_lb$points[which(crowdAveragedProjectionsWithActuals_weekly_lb$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_lb$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_lb$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_lb_crowd_subset <- round(accuracyIndices_weekly_lb_crowd_subset$rsquared, digits = 2)
me_weekly_lb_crowd_subset <- round(accuracyIndices_weekly_lb_crowd_subset$ME, digits = 2)
mae_weekly_lb_crowd_subset <- round(accuracyIndices_weekly_lb_crowd_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_db <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_db$raw_points,
  actual = projectionsWithActuals_weekly_db$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_db <- round(accuracyIndices_weekly_db$rsquared, digits = 2)
me_weekly_db <- round(accuracyIndices_weekly_db$ME, digits = 2)
mae_weekly_db <- round(accuracyIndices_weekly_db$MAE, digits = 2)
```

A calibration plot of the weekly projections for Linebackers is in @fig-calibrationPlotWeeklyDBs.
...
accuracyIndices_weekly_db_subset <- petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_db$raw_points[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = projectionsWithActuals_weekly_db$fantasyPoints[which(projectionsWithActuals_weekly$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_db_subset <- round(accuracyIndices_weekly_db_subset$rsquared, digits = 2)
me_weekly_db_subset <- round(accuracyIndices_weekly_db_subset$ME, digits = 2)
mae_weekly_db_subset <- round(accuracyIndices_weekly_db_subset$MAE, digits = 2)
```
...
accuracyIndices_weekly_db_crowd <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_db$points,
  actual = crowdAveragedProjectionsWithActuals_weekly_db$fantasyPoints,
  dropUndefined = TRUE
)

rsquare_weekly_db_crowd <- round(accuracyIndices_weekly_db_crowd$rsquared, digits = 2)
me_weekly_db_crowd <- round(accuracyIndices_weekly_db_crowd$ME, digits = 2)
mae_weekly_db_crowd <- round(accuracyIndices_weekly_db_crowd$MAE, digits = 2)
```
...
accuracyIndices_weekly_db_crowd_subset <- petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_db$points[which(crowdAveragedProjectionsWithActuals_weekly_db$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  actual = crowdAveragedProjectionsWithActuals_weekly_db$fantasyPoints[which(crowdAveragedProjectionsWithActuals_weekly_db$player_id_season_week %in% playersWithHighProjectedOrActualPoints_weekly)],
  dropUndefined = TRUE
)

rsquare_weekly_db_crowd_subset <- round(accuracyIndices_weekly_db_crowd_subset$rsquared, digits = 2)
me_weekly_db_crowd_subset <- round(accuracyIndices_weekly_db_crowd_subset$ME, digits = 2)
mae_weekly_db_crowd_subset <- round(accuracyIndices_weekly_db_crowd_subset$MAE, digits = 2)
```

### Summarizing the Accuracy By Position {#sec-predictionAccuracyByPosition}

```{r}
#| include: false

seasonalAccuracy <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
...
seasonalAccuracy_subset <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
  ...
  rsquared = c(
    rsquare_seasonal_overall_subset,
    rsquare_seasonal_qb_subset,
    rsquare_seasonal_rb_subset,
    rsquare_seasonal_wr_subset,
    rsquare_seasonal_te_subset,
    rsquare_seasonal_k_subset,
    rsquare_seasonal_dl_subset,
    rsquare_seasonal_lb_subset,
    rsquare_seasonal_db_subset
  ),
  me = c(
    me_seasonal_overall_subset,
    me_seasonal_qb_subset,
    me_seasonal_rb_subset,
    me_seasonal_wr_subset,
    me_seasonal_te_subset,
    me_seasonal_k_subset,
    me_seasonal_dl_subset,
    me_seasonal_lb_subset,
    me_seasonal_db_subset
  ),
  mae = c(
    mae_seasonal_overall_subset,
    mae_seasonal_qb_subset,
    mae_seasonal_rb_subset,
    mae_seasonal_wr_subset,
    mae_seasonal_te_subset,
    mae_seasonal_k_subset,
    mae_seasonal_dl_subset,
    mae_seasonal_lb_subset,
    mae_seasonal_db_subset
  )
)

seasonalAccuracy_crowd <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
  ...
  rsquared = c(
    rsquare_seasonal_overall_crowd,
    rsquare_seasonal_qb_crowd,
    rsquare_seasonal_rb_crowd,
    rsquare_seasonal_wr_crowd,
    rsquare_seasonal_te_crowd,
    rsquare_seasonal_k_crowd,
    rsquare_seasonal_dl_crowd,
    rsquare_seasonal_lb_crowd,
    rsquare_seasonal_db_crowd
  ),
  me = c(
    me_seasonal_overall_crowd,
    me_seasonal_qb_crowd,
    me_seasonal_rb_crowd,
    me_seasonal_wr_crowd,
    me_seasonal_te_crowd,
    me_seasonal_k_crowd,
    me_seasonal_dl_crowd,
    me_seasonal_lb_crowd,
    me_seasonal_db_crowd
  ),
  mae = c(
    mae_seasonal_overall_crowd,
    mae_seasonal_qb_crowd,
    mae_seasonal_rb_crowd,
    mae_seasonal_wr_crowd,
    mae_seasonal_te_crowd,
    mae_seasonal_k_crowd,
    mae_seasonal_dl_crowd,
    mae_seasonal_lb_crowd,
    mae_seasonal_db_crowd
  )
)

seasonalAccuracy_crowd_subset <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
  ...
  rsquared = c(
    rsquare_seasonal_overall_crowd_subset,
    rsquare_seasonal_qb_crowd_subset,
    rsquare_seasonal_rb_crowd_subset,
    rsquare_seasonal_wr_crowd_subset,
    rsquare_seasonal_te_crowd_subset,
    rsquare_seasonal_k_crowd_subset,
    rsquare_seasonal_dl_crowd_subset,
    rsquare_seasonal_lb_crowd_subset,
    rsquare_seasonal_db_crowd_subset
  ),
  me = c(
    me_seasonal_overall_crowd_subset,
    me_seasonal_qb_crowd_subset,
    me_seasonal_rb_crowd_subset,
    me_seasonal_wr_crowd_subset,
    me_seasonal_te_crowd_subset,
    me_seasonal_k_crowd_subset,
    me_seasonal_dl_crowd_subset,
    me_seasonal_lb_crowd_subset,
    me_seasonal_db_crowd_subset
  ),
  mae = c(
    mae_seasonal_overall_crowd_subset,
    mae_seasonal_qb_crowd_subset,
    mae_seasonal_rb_crowd_subset,
    mae_seasonal_wr_crowd_subset,
    mae_seasonal_te_crowd_subset,
    mae_seasonal_k_crowd_subset,
    mae_seasonal_dl_crowd_subset,
    mae_seasonal_lb_crowd_subset,
    mae_seasonal_db_crowd_subset
  )
)

weeklyAccuracy <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
...
weeklyAccuracy_subset <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
  ...
  rsquared = c(
    rsquare_weekly_overall_subset,
    rsquare_weekly_qb_subset,
    rsquare_weekly_rb_subset,
    rsquare_weekly_wr_subset,
    rsquare_weekly_te_subset,
    rsquare_weekly_k_subset,
    rsquare_weekly_dl_subset,
    rsquare_weekly_lb_subset,
    rsquare_weekly_db_subset
  ),
  me = c(
    me_weekly_overall_subset,
    me_weekly_qb_subset,
    me_weekly_rb_subset,
    me_weekly_wr_subset,
    me_weekly_te_subset,
  ...
  mae = c(
    mae_weekly_overall_subset,
    mae_weekly_qb_subset,
    mae_weekly_rb_subset,
    mae_weekly_wr_subset,
    mae_weekly_te_subset,
    mae_weekly_k_subset,
    mae_weekly_dl_subset,
    mae_weekly_lb_subset,
    mae_weekly_db_subset
  )
)

weeklyAccuracy_crowd <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
  ...
  rsquared = c(
    rsquare_weekly_overall_crowd,
    rsquare_weekly_qb_crowd,
    rsquare_weekly_rb_crowd,
    rsquare_weekly_wr_crowd,
    rsquare_weekly_te_crowd,
    rsquare_weekly_k_crowd,
    rsquare_weekly_dl_crowd,
    rsquare_weekly_lb_crowd,
    rsquare_weekly_db_crowd
  ),
  me = c(
    me_weekly_overall_crowd,
    me_weekly_qb_crowd,
    me_weekly_rb_crowd,
    me_weekly_wr_crowd,
    me_weekly_te_crowd,
...
weeklyAccuracy_crowd_subset <- data.frame(
  position = c(
    "All Positions",
    "Quarterbacks",
    "Running Backs",
  ...
  rsquared = c(
    rsquare_weekly_overall_crowd_subset,
    rsquare_weekly_qb_crowd_subset,
    rsquare_weekly_rb_crowd_subset,
    rsquare_weekly_wr_crowd_subset,
    rsquare_weekly_te_crowd_subset,
    rsquare_weekly_k_crowd_subset,
    rsquare_weekly_dl_crowd_subset,
    rsquare_weekly_lb_crowd_subset,
    rsquare_weekly_db_crowd_subset
  ),
  me = c(
    me_weekly_overall_crowd_subset,
    me_weekly_qb_crowd_subset,
    me_weekly_rb_crowd_subset,
    me_weekly_wr_crowd_subset,
    me_weekly_te_crowd_subset,
    me_weekly_k_crowd_subset,
    me_weekly_dl_crowd_subset,
    me_weekly_lb_crowd_subset,
    me_weekly_db_crowd_subset
  ),
  mae = c(
    mae_weekly_overall_crowd_subset,
    mae_weekly_qb_crowd_subset,
    mae_weekly_rb_crowd_subset,
    mae_weekly_wr_crowd_subset,
    mae_weekly_te_crowd_subset,
    mae_weekly_k_crowd_subset,
    mae_weekly_dl_crowd_subset,
    mae_weekly_lb_crowd_subset,
    mae_weekly_db_crowd_subset
  )
)
```

We summarize the accuracy of seasonal projections by position in Tables [-@tbl-seasonalAccuracyByPosition] and [-@tbl-seasonalAccuracyByPositionSubset].

```{r}
#| label: tbl-seasonalAccuracyByPosition
#| tbl-cap: "Accuracy of Seasonal Projections By Position."
#| echo: false

knitr::kable(
  seasonalAccuracy,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```

```{r}
#| label: tbl-seasonalAccuracyByPositionSubset
#| tbl-cap: "Accuracy of Seasonal Projections By Position, Among Players with Greater than 100 Projected or Actual Points."
#| echo: false

knitr::kable(
  seasonalAccuracy_subset,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```

We summarize the accuracy of crowd-averaged seasonal projections by position in Tables [-@tbl-seasonalCrowdAccuracyByPosition] and [-@tbl-seasonalCrowdAccuracyByPositionSubset].

```{r}
#| label: tbl-seasonalCrowdAccuracyByPosition
#| tbl-cap: "Accuracy of Crowd-Averaged Seasonal Projections By Position."
#| echo: false

knitr::kable(
  seasonalAccuracy_crowd,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```

```{r}
#| label: tbl-seasonalCrowdAccuracyByPositionSubset
#| tbl-cap: "Accuracy of Crowd-Averaged Seasonal Projections By Position, Among Players with Greater than 100 Projected or Actual Points."
#| echo: false

knitr::kable(
  seasonalAccuracy_crowd_subset,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```

We summarize the accuracy of weekly projections by position in Tables [-@tbl-weeeklyAccuracyByPosition] and [-@tbl-weeklyAccuracyByPositionSubset].

```{r}
#| label: tbl-weeeklyAccuracyByPosition
#| tbl-cap: "Accuracy of Weekly Projections By Position."
#| echo: false
...
```{r}
#| label: tbl-weeklyAccuracyByPositionSubset
#| tbl-cap: "Accuracy of Weekly Projections By Position, Among Players with Greater than 6 Projected or Actual Points."
#| echo: false

knitr::kable(
  weeklyAccuracy_subset,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```

We summarize the accuracy of crowd-averaged seasonal projections by position in Tables [-@tbl-weeklyCrowdAccuracyByPosition] and [-@tbl-weeklyCrowdAccuracyByPositionSubset].

```{r}
#| label: tbl-weeklyCrowdAccuracyByPosition
#| tbl-cap: "Accuracy of Crowd-Averaged Weekly Projections By Position."
#| echo: false

knitr::kable(
  weeklyAccuracy_crowd,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```

```{r}
#| label: tbl-weeklyCrowdAccuracyByPositionSubset
#| tbl-cap: "Accuracy of Crowd-Averaged Weekly Projections By Position, Among Players with Greater than 6 Projected or Actual Points."
#| echo: false

knitr::kable(
  weeklyAccuracy_crowd_subset,
  col.names = c("Position","$R^2$","Mean Error","Mean Absolute Error"),
  booktabs = TRUE)
```
...
For seasonal projections, in terms of comparison by position, projections showed greatest discrimination ($R^2$) for Quarterbacks, followed by Running Backs/Tight Ends, and Wide Receivers.
Predictions for defensive players and kickers showed a negative $R^2$, indicating that projections would have been more accurate merely by predicting the mean of the actual values for every observation (i.e., [predicting from the base rate](#sec-predictingFromBaseRate)).
In terms of comparison by projection source, crowd-averaged projections showed greater accuracy than individual projection sources (i.e., "experts") in terms of discrimination ($R^2$) and calibration (MAE).
This was especially the case when examining individual sources separately (rather than in one combined object).

For weekly projections, in terms of comparison by position, projections showed greatest discrimination ($R^2$) for Running Backs, followed by Wide Receivers, Quarterbacks, and Tight Ends.
Predictions for defensive players and kickers showed a negative $R^2$, indicating that projections would have been more accurate merely by predicting the mean of the actual values for every observation (i.e., [predicting from the base rate](#sec-predictingFromBaseRate)).
In terms of comparison by projection source, crowd-averaged projections showed greater accuracy than individual projection sources in terms of discrimination ($R^2$) and calibration (MAE).

In summary, we found that, for both seasonal and weekly projections, crowd-averaged projections showed greater accuracy than individual projection sources (i.e., "experts"), consistent with prior findings [@Kartes2024\; archived at <https://perma.cc/69F7-LLTN>; @Petersen2017\; archived at <https://perma.cc/BG2W-ANUF>].
Nevertheless projections—including individual sources and crowd-averaged—were not highly accurate.
Projections tended not to explain more than about two-thirds of the variance in players' performance, and frequently explained well less than half of the variance, particularly when distinguishing between players with high (i.e, more than 100) projected or actual points—who are the players that are the most important to distinguish, because those are the players that you are most likely trying to decide between when drafting.
Thus, although projections for offensive positions (besides Kickers) do better than chance, there remains considerable room for improvement in predictive accuracy.
The modest accuracy of projections likely reflects that human behavior is multiply determined and hard to predict.

Prediction accuracy tended to be higher for seasonal projections than for weekly projections—likely because of the greater number of "trials" (i.e., plays) in a season compared to a game, so more of the random error averages out.
The weaker accuracy weekly projections makes betting on daily fantasy sports a gamble.

The association between projected and actual fantasy points tended to flatten at the higher levels of projected points, for some positions, including Running Backs, Kickers, and defensive players.
A consequence of this pattern, from the perspective of draft strategy, is that it would not make sense to draft players projected to score more than the flattening of the projected–actual curve.
For instance, drafting a highly routed Wide Receiver may be safer than drafting a highly ranked Running Back, who may be more prone to injury.
This finding is consistent with findings from @Petersen2017 [archived at <https://perma.cc/BG2W-ANUF>], who found that projections were more accurate for Quarterbacks and Wide Receivers than for Running Backs; projections were the least accurate for Kickers, Defensive Backs, and Defense/Special Teams.
These finding were also extended by @Kartes2025 (archived at <https://perma.cc/R3WS-CAKD>) whose evaluation of the accuracy of projections by position found that:

a. Quarterbacks with preseason ranks of 6–10 showed the greatest systematic underperformance relative to their projections, suggesting that you should either draft an elite Quarterback (ranked 1–5) or wait until later rounds to draft multiple lower-ranked Quarterbacks.
a. Elite Running Backs (with preseason ranks of 1–5) showed strong underperformance (relative to projections), suggesting that you should draft Running Backs ranked 6 and below.
a. Wide Receivers showed among the more consistent and predictable patterns in projections, suggesting that it may be safer to draft a top Wide Receiver than top-ranked players at other positions.
a. Tight Ends showed relative inaccuracy of projections with limited upside concentrated among a few players, which means it is difficult, at the outset, to predict which Tight Ends will excel; based on this, @Kartes2025 suggested avoiding overpaying for mid-tier Tight Ends; to either draft a Tight End early or to wait until later rounds.

This does *not* mean that you should intentionally draft a 6th ranked Running Back instead of the top-ranked Running Back.
Instead, what it may mean is that, instead of drafting the top-ranked Running Back, you may be better off drafting the top-ranked Wide Receiver and to get a value player at Running Back in a subsequent pick.
...
Thus, I would not put much confidence in the predictions by fantasy football pundits.
The most common pattern of experts' predictions is that they show overextremity, that is, their predictions have probability judgments that tend to be too extreme, as described in @sec-calibration.
...
- Instead of aggregating scores from multiple variables, another option is to "unamalgamate" the predictors, that is, to use each variable as a separate predictor, to maximize predictive power [@Trafimow2025].
...
In terms of the accuracy of historical fantasy football projections, projections showed moderate accuracy in terms of [discrimination](#sec-discrimination) in predicting players' fantasy performance.
However, projections were not highly accurate.
Projections showed some [miscalibration](#sec-calibrationIndices) with respect to actual fantasy points.
Projection sources appeared to be [overconfident](#sec-cognitiveBiasesOverconfidence) in their projections of player performance.
In general, projected showed a modest tendency to underpredict performance at lower levels of projected points and a moderate-to-strong tendency overpredict performance at higher levels of projected points, an example of the overextremity, which is an example of the overprecision form of [overconfidence](#sec-cognitiveBiasesOverconfidence).
Crowd-averaged projections tend to be more accurate than individual sources of projections.
