...
The `nfl_depthCharts` object is in `id`-`season`-`week`-`position` form.
...
Let's check for duplicate `id`-`season`-`week`-`position` instances:
...
Let's check again for duplicate `id`-`season`-`week`-`position` instances:
...
The `nfl_expectedFantasyPoints_weekly` object is in `game`-`player` form and in `season`-`week`-`player` form.
That is, each row should be uniquely identified by the combination of `game_id` and `player_id`.
Each row should also be uniquely identified by the combination of `season`, `week`, and player_id.
Let's rearrange the data accordingly:

```{r}
nfl_expectedFantasyPoints_weekly <- nfl_expectedFantasyPoints_weekly_raw %>% 
  select(season, week, game_id, player_id, full_name, posteam, position, everything()) %>% 
...
The `nfl_actualStats_offense_career`, `nfl_actualStats_defense_career`, and `nfl_actualStats_kicking_career` objects are in `player` form.
