...
To create a vector, use the `c()` function to combine elements into a vector ("c" stands for combine).
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] has a convenience function, `%ni%`, for the logical operator of determining whether an element is *not* in a value of another vector:
...
The operator, `which()`, can be used to obtain the TRUE indices of an object, which is useful for [subsetting](#sec-subset).
...
We widen the data by two variables (`season` and `week`), using `tidyverse`, so that the data are now in "`player` form" (where each row is uniquely identified by the `ID` variable):
