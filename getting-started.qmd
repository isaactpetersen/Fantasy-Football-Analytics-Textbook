...
# Reshape from wide to long format
nfl_schedules_long <- nfl_schedules %>% 
  pivot_longer(
...
nfl_actualStats_offense_weekly_schedules <- nfl_actualStats_offense_weekly_schedules_long %>%
...
player_birthdate_gameday <- left_join(
  nfl_actualStats_offense_weekly_schedules,
  unique(nfl_players[,c("gsis_id","birth_date")]),
  by = c("player_id" = "gsis_id")
)

player_birthdate_gameday$birth_date <- ymd(player_birthdate_gameday$birth_date)
player_birthdate_gameday$gameday <- ymd(player_birthdate_gameday$gameday)

# Calculate player's age for a given week as the difference between their birthdate and the game date
player_birthdate_gameday$age <- interval(
  start = player_birthdate_gameday$birth_date,
  end = player_birthdate_gameday$gameday
) %>% 
  time_length(unit = "years")

# Merge with player info
player_age <-  left_join(
  player_birthdate_gameday,
...
player_age_stats <- left_join(
  player_age,
  nfl_actualStats_offense_weekly,
  by = c(c("season","week","player_id","game_id")))

player_age_stats$years_of_experience <- as.integer(player_age_stats$years_of_experience)
```
