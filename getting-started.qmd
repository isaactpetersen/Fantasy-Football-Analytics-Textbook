...
The `nfl_players` object is in `id` form.
...
The `nfl_teams` object is in `id` form.
...
The `nfl_rosters` object is in `id`-`season`-`team` form.
...
The `nfl_rosters_weekly` object is in `id`-`season`-`week` form.
...
The `nfl_schedules` object is in `id` form.
...
The `nfl_combine` object is in `id` form.
That is, each row should be uniquely identified by their `id`.
...
The `nfl_draftPicks` object is in `id` form.
That is, each row should be uniquely identified by `gsis_id`.
Let's rearrange the data accordingly:
...
The `nfl_pbp` object is in `game_id`-`drive`-`play_id` form.
