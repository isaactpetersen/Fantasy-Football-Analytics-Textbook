...
distinct_seasons_offense <- player_stats_weekly_offense %>%
  ...
  mutate(total_years_of_experience = as.integer(years_of_experience)) %>% 
...
years_of_experience_offense <- distinct_seasons_offense %>% 
  ...
  group_by(player_id) %>%
  mutate(years_of_experience = first(total_years_of_experience) - (row_number() - 1)) %>%
  ungroup()
...
player_stats_seasonal_offense <- full_join(
  nfl_actualStats_offense_seasonal,
  nfl_players %>% select(-position, -position_group, -season),
  by = c("player_id" = "gsis_id")
)

player_stats_seasonal_defense <- full_join(
  nfl_actualStats_defense_seasonal,
  nfl_players %>% select(-position, -position_group, -season),
  by = c("player_id" = "gsis_id")
)

player_stats_seasonal_kicking <- full_join(
