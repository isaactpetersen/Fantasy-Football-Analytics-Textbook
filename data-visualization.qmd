...
Graphics serve an important way of efficiently communicating findings to diverse audiences.
Effective graphics can help people with less technical knowledge better understand your findings.
As noted by @Tanney2021, it is important to communicate your work clearly to stakeholders (e.g., coaches, companies, or prospective clients); if you cannot explain your work clearly, you cannot expect a stakeholder to "buy in" to what you are selling.
...
@AdobeExpress2020 describes important principles about graphic design at the following link: <https://www.adobe.com/express/learn/blog/8-basic-design-principles-to-help-you-create-better-graphics> (archived at <https://perma.cc/29P9-NNSK>).
...
"Showing the data" involves showing the data that matter the most.
...
The Data Visualization Catalogue, from Data to Viz, and Data Viz Project provide examples of various types of plots depending on one's goal: <https://datavizcatalogue.com/search.html>, <https://www.data-to-viz.com>, and <https://datavizproject.com>.
...
We will create the plots using the `ggplot2` package [@R-ggplot2; @ggplot3rdEd].
...
Good color schemes for sequential, diverging, and qualitative (i.e., categorical) data are provided by [ColorBrewer](https://colorbrewer2.org) (<https://colorbrewer2.org>) and are available using the `ggplot2::scale_color_brewer()` and `ggplot2::scale_fill_brewer()` functions of the `ggplot2` package [@R-ggplot2], as demonstrated in @fig-barPlotColorScheme (@sec-barPlotModifiedColorScheme).
[ColorBrewer](https://colorbrewer2.org) allows users to select colorblind safe, print friendly, and photocopy safe palettes.
Good colorblind-friendly color schemes for binned (`_b`), continuous (`_c`), and discrete (i.e., categorical; `_d`) scales are available from the [`viridis` palette](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html) using the `ggplot2::scale_color_viridis_X()` and `ggplot2::scale_fill_viridis_X()` functions of the `ggplot2` package [@R-ggplot2] (where `X` is `b`, `c`, or `d`, depending on whether you want a binned, continuous, discrete scale). 
There are a variety of resources for color schemes that are accessible to color-blind viewers:

- <https://www.datylon.com/blog/data-visualization-for-colorblind-readers> [@Kilin2022; archived at <https://perma.cc/7VTA-Y8YS>]
- the `viridis` package [@R-viridis; @Garnier2024a_packages]
  - <https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html> (archived at <https://perma.cc/NK9K-HL7L>)
- [Okabe-Ito color palette](https://jfly.uni-koeln.de/color/) [@Okabe2008; archived at <https://perma.cc/VD2E-XZZU>]
- @Nunez2018

The Okabe-Ito color palette [@Okabe2008] can be specified with the following colors:
...
The Okabe-Ito color palette [@Okabe2008] is depicted in Figures [-@fig-okabeItoPaletteGray] and [-@fig-okabeItoPaletteBlack].

```{r}
#| label: fig-okabeItoPaletteGray
#| fig-cap: "Okabe-Ito Color Palette With Gray."
#| fig-alt: "Okabe-Ito Color Palette With Gray."

graphics::pie(rep(1, 8), col = palette_OkabeIto)
```

```{r}
#| label: fig-okabeItoPaletteBlack
#| fig-cap: "Okabe-Ito Color Palette With Black."
#| fig-alt: "Okabe-Ito Color Palette With Black."

graphics::pie(rep(1, 8), col = palette_OkabeIto_black)
```

## `ggplot2` Syntax {#sec-ggplot2Syntax}

### General Syntax {#sec-ggplot2SyntaxGeneral}

The `ggplot2` package [@R-ggplot2] is based on the grammar of graphics [@ggplot3rdEd], which is the principle that you can build any graph from three components: 1) the data, 2) the coordinate system (and aesthetic attributes), and 3) the geometric objects ("geoms")—visual marks that represent data points (e.g., points, lines, and bars).
The general syntax for plots created using the `ggplot2` package [@R-ggplot2] is:
...
The `ggplot2::coord_cartesian()` function can be used to specify the axis limits; however, this function is not required and the plot will apply sensible limits by default.
In addition, there are optional settings you can specify or modify, such as [axis titles](#sec-scatterplotPlotLabels), [axis labels](#sec-scatterplotAxes), [faceting](#sec-faceting), and the [theme](#sec-scatterplotTheme).
...
You can layer multiple geoms in the same plot, layer by layer.
You can have a different data set and aesthetics associated with each geom, by specifying the `data` and `mapping` arguments for the function of that particular geom.
For an example of a plot that uses different data sets for different geoms, see @fig-bootstrapRegressionCoefficients (@sec-multipleRegressionRobustRegression).

### Geometric Objects {#sec-ggplot2SyntaxGeoms}

Here is a list of common geometric objects ("geoms") available in `ggplot2` (there are many others):

- [histogram](#sec-histogram): `ggplot2::geom_histogram()`
- [density plot](#sec-densityPlot): `ggplot2::geom_density()`
- [box-and-whisker plot](#sec-boxAndWhiskerPlot): `ggplot2::geom_boxplot()`
- [violin plot](#sec-violinPlot): `ggplot2::geom_violin()`
- [bar plot](#sec-barPlot): `ggplot2::geom_bar()`
- [scatterplot](#sec-scatterplot): `ggplot2::geom_point()`
- [rug plot](#sec-scatterplotRugPlot): `ggplot2::geom_rug()`
- [line chart](#sec-lineChart): `ggplot2::geom_line()`
- statistical summary (e.g., [best-fit line](#sec-scatterplotBestFitLine)): `ggplot2::geom_smooth()`

### Color {#sec-ggplot2SyntaxColor}

In `ggplot2`, you can specify the color of a geom using one of two parameters: 1) `fill` and 2) `color`.
The `fill` parameter specifies the color inside the geom.
The `color` parameter specifies the outline/border color of the geom.

The colors to use in the `fill` and `color` parameters can be specified by name (as in @fig-colorByName), hexadecimal (HEX) codes (as in @fig-colorByHEX), values in red-green-blue (RGB) space (as in @fig-colorByRGB), or palettes (e.g., ColorBrewer or viridis; as in Figures [-@fig-multipleColorsByPaletteColorBrewer] and [-@fig-multipleColorsByPaletteViridis]).

```{r}
#| label: fig-colorByName
#| fig-cap: "Specifying the Color/Fill by Name."
#| fig-alt: "Specifying the Color/Fill by Name."
...
However, you can use any color by specifying its HEX code, as in @fig-colorByHEX.

```{r}
#| label: fig-colorByHEX
#| fig-cap: "Specifying the Color/Fill by Hexadecimal Code."
#| fig-alt: "Specifying the Color/Fill by Hexadecimal Code."
...
You can use any color by specifying its values in red-green-blue (RGB) color space using the `grDevices::rgb()` function, as in @fig-colorByRGB.

```{r}
#| label: fig-colorByRGB
#| fig-cap: "Specifying the Color/Fill by Value in Red-Green-Blue (RGB) Space."
#| fig-alt: "Specifying the Color/Fill by Value in Red-Green-Blue (RGB) Space."
...
We can allow the colors to differ by group by specifying the `fill` parameter in the mapping aesthetics, as in @fig-colorByGroup.

```{r}
#| label: fig-colorByGroup
#| fig-cap: "Specifying Color By Group."
#| fig-alt: "Specifying Color By Group."
...
We can specify multiple colors manually using the `ggplot2::scale_fill_manual()` function, as in @fig-multipleColorsManually.

```{r}
#| label: fig-multipleColorsManually
#| fig-cap: "Specifying Multiple Colors Manually."
#| fig-alt: "Specifying Multiple Colors Manually."
...
We can also specify the colors using a palette, such as by the ColorBrewer palette using the `ggplot2::scale_fill_brewer()` function (as in @fig-multipleColorsByPaletteColorBrewer) or the viridis palette using the `ggplot2::scale_fill_viridis_d()` function (as in @fig-multipleColorsByPaletteViridis).

```{r}
#| label: fig-multipleColorsByPaletteColorBrewer
#| fig-cap: "Specifying Multiple Colors Using the ColorBrewer Palette."
#| fig-alt: "Specifying Multiple Colors Using the ColorBrewer Palette."
...
```{r}
#| label: fig-multipleColorsByPaletteViridis
#| fig-cap: "Specifying Multiple Colors Using the Viridis Palette."
#| fig-alt: "Specifying Multiple Colors Using the Viridis Palette."
...
### Line Type {#sec-ggplot2SyntaxLineType}

Common line types include "solid" lines, "dashed" lines, and "dotted" lines.
A full list of line types available is provided here: <https://ggplot2.tidyverse.org/articles/ggplot2-specs.html> (archived at <https://perma.cc/N9ED-6LUS>).
To specify the line type, you can use the `linetype` parameter, as in @fig-lineType.

```{r}
#| label: fig-lineType
#| fig-cap: "Specifying Line Type."
#| fig-alt: "Specifying Line Type."
...
We can allow the line type to differ by group by specifying the `linetype` parameter in the mapping aesthetics, as in @fig-lineTypeByGroup.

```{r}
#| label: fig-lineTypeByGroup
#| fig-cap: "Specifying Line Type By Group."
#| fig-alt: "Specifying Line Type By Group."
...
We can specify the line types manually using the `ggplot2::scale_linetype_manual()` function as in @fig-lineTypeByGroupManually.

```{r}
#| label: fig-lineTypeByGroupManually
#| fig-cap: "Specifying Line Type By Group Manually."
#| fig-alt: "Specifying Line Type By Group Manually."
...
### Line Width {#sec-ggplot2SyntaxLineWidth}

To specify the line width, you can use the `linewidth` parameter, as in @fig-linewidth.

```{r}
#| label: fig-linewidth
#| fig-cap: "Specifying Line Width."
#| fig-alt: "Specifying Line Width."
...
### Point Shape {#sec-ggplot2SyntaxPointShape}

Common point shapes include circles, triangles, squares, etc.
A full list of point shapes available is provided here: <https://ggplot2.tidyverse.org/articles/ggplot2-specs.html> (archived at <https://perma.cc/N9ED-6LUS>) and is in @fig-pointShapeOptions.

::: {#fig-pointShapeOptions}
![](images/pointShapes.png){width=100% fig-alt="Available Point Shapes in ggplot2."}

Available Point Shapes in ggplot2.
:::

To specify the point shape, you can use the `shape` parameter, as in @fig-pointShape.

```{r}
#| label: fig-pointShape
#| fig-cap: "Specifying Point Shape."
#| fig-alt: "Specifying Point Shape."
...
We can allow the point shape to differ by group by specifying the `shape` parameter in the mapping aesthetics, as in @fig-pointShapeByGroup.

```{r}
#| label: fig-pointShapeByGroup
#| fig-cap: "Specifying Point Shape By Group."
#| fig-alt: "Specifying Point Shape By Group."
...
We can specify the point shape manually using the `ggplot2::scale_shape_manual()` function as in @fig-pointShapeByGroupManually.

```{r}
#| label: fig-pointShapeByGroupManually
#| fig-cap: "Specifying Point Shape By Group Manually."
#| fig-alt: "Specifying Point Shape By Group Manually."
...
### Size of Points and Text {#sec-ggplot2SyntaxSize}

To specify the size of points and text, you can use the `size` and `base_size` parameters, respectively, as in @fig-sizePointsText.

```{r}
#| label: fig-sizePointsText
#| fig-cap: "Specifying Size of Points and Text."
#| fig-alt: "Specifying Size of Points and Text."
...
You can also specify the font size separately for particular elements, as in @fig-sizeTextManually.

```{r}
#| label: fig-sizeTextManually
#| fig-cap: "Specifying Size of Text Manually for Particular Elements."
#| fig-alt: "Specifying Size of Text Manually for Particular Elements."
...
@Scherer2021 describes various ways of visualizing univariate distributions; see [here](https://z3tt.github.io/beyond-bar-and-box-plots) (archived at <https://perma.cc/EEJ8-LND2>).
...
A ridgeline plot of fantasy points is depicted in @fig-ridgeline1 using the `ggridges` package [@R-ggridges].
...
Second, we create the base layer of the plot using the `ggplot2::ggplot()` function of the `ggplot2` package [@R-ggplot2], as in @fig-scatterplot1.
...
Third, we create a scatterplot using the `ggplot2::geom_point()` function from the `ggplot2` package [@R-ggplot2], as in @fig-scatterplot2:
...
For a list of themes available in `ggplot2`, see here: <https://ggplot2-book.org/themes#sec-themes> [@ggplot3rdEd].
...
After creating our plot, we can make the plot interactive using the `plotly::ggplotly()` function from the `plotly` package [@R-plotly; @Sievert2020_packages], as in @fig-scatterplot11.
...
After creating our plot, we can make the plot interactive using the `plotly::ggplotly()` function from the `plotly` package [@R-plotly; @Sievert2020_packages].
...
### Scatterplot with Rug Plot {#sec-scatterplotRugPlot}

We can also add a rug plot, as in @fig-scatterplot12.

```{r}
#| label: fig-scatterplot12
#| fig-cap: "Scatterplot With Rug Plot."
#| fig-alt: "Scatterplot With Rug Plot."
...
### Marginal Density/Histogram {#sec-scatterplotMarginalDensigram}

We can also add a marginal density/histogram, as in @fig-scatterplot13 using the `ggExtra` package [@R-ggExtra]:

```{r}
#| label: fig-scatterplot13
#| fig-cap: "Scatterplot With Marginal Density/Histogram."
#| fig-alt: "Scatterplot With Marginal Density/Histogram."
...
Or we can create the plots manually, as in @fig-scatterplot14 using the `patchwork` package [@R-patchwork]:

```{r}
#| label: fig-scatterplot14
#| fig-cap: "Scatterplot With Marginal Density/Histogram."
#| fig-alt: "Scatterplot With Marginal Density/Histogram."

dens1 <- ggplot2::ggplot(
  data = rb_seasonal,
  aes(
    x = age)) +
  coord_cartesian(
...
We use the `gghighlight` package [@R-gghighlight] to highlight particular elements:
...
#| fig-alt: "Rushing Yards Per Carry (Season) by Player Age."
...
#| fig-alt: "NFL Rushing Yards Per Carry Per Season by Player Age."
...
#| fig-alt: "Rushing Expected Points Added (Season) by Player Age."
...
#| fig-alt: "NFL Rushing Expected Points Added (EPA) During a Season by Player Age."
...
Defensive EPA per play during the 2024 NFL season is depicted as a function of offensive EPA per play in @fig-plotDefOffEPA using the `nflplotR` package [@R-nflplotR].

```{r}
#| label: fig-plotDefOffEPA
#| fig-cap: "2024 NFL Offensive and Defensive EPA Per Play."
#| fig-alt: "2024 NFL Offensive and Defensive EPA Per Play."
...
This chapter provided an overview of principles of data visualization and how to implement them using `R`.
Key principles of data visualization include: ensuring adequate contrast, highlighting key elements of interest (and using gray for other elements), reducing clutter, and adding helpful titles and annotations.
We provided examples of data visualizations, including [histograms](#sec-histogram), [density plots](#sec-densityPlot), [rug plots](#sec-densityHistogram), [box-and-whisker plots](#sec-boxAndWhiskerPlot), [violin plots](#sec-violinPlot), [ridgeline plots](#sec-ridgelinePlot), [scatterplots](#sec-scatterplot), [line charts](#sec-lineChart), and [bar plots](#sec-barPlot).
