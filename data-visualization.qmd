...
The `coord_cartesian()` function can be used to specify the axis limits; however, this function is not required and the plot will apply sensible limits by default.
...
You can see a full list of color names available using the `colors()` function:
...
You can use any color by specifying its values in red-green-blue (RGB) color space using the `rgb()` function, as in @fig-colorByRGB.
...
We can specify multiple colors manually using the `scale_fill_manual()` function, as in @fig-multipleColorsManually.
...
We can also specify the colors using a palette, such as by the ColorBrewer palette using the `scale_fill_brewer()` function (as in @fig-multipleColorsByPaletteColorBrewer) or the viridis palette using the `scale_fill_viridis_d()` function (as in @fig-multipleColorsByPaletteViridis).
...
We can specify the line types manually using the `scale_linetype_manual()` function as in @fig-lineTypeByGroupManually.
...
We can specify the point shape manually using the `scale_shape_manual()` function as in @fig-pointShapeByGroupManually.
...
Second, we create the base layer of the plot using the `ggplot()` function of the `ggplot2` package [@R-ggplot2], as in @fig-scatterplot1.
...
Third, we create a scatterplot using the `geom_point()` function from the `ggplot2` package [@R-ggplot2], as in @fig-scatterplot2:
...
After creating our plot, we can make the plot interactive using the `ggplotly()` function from the `plotly` package [@R-plotly; @Sievert2020_packages], as in @fig-scatterplot11.
...
After creating our plot, we can make the plot interactive using the `ggplotly()` function from the `plotly` package [@R-plotly; @Sievert2020_packages].
