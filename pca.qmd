...
dataMergedWithPCA |> 
  arrange(-PC1) |> 
  select(player_display_name, season, week, PC1, all_of(component1vars)) |> 
  na.omit() |> 
  head()
```
...
dataMergedWithPCA |> 
  arrange(-PC2) |> 
  select(player_display_name, season, week, PC2, all_of(component2vars)) |> 
  na.omit() |> 
  head()
```
...
dataMergedWithPCA |> 
  arrange(-PC3) |> 
  select(player_display_name, season, week, PC3, all_of(component3vars)) |> 
  na.omit() |> 
  head()
```
...
There are many [decisions to make in PCA](#sec-pcaDecisions).
These decisions can have important impacts on the resulting solution.
Thus, it can be helpful for theory and interpretability to help guide decision-making when conducting PCA.
