...
An actuarial formula for projecting a Running Backs rushing yards might be something like @eq-actuarialFantasy:
...
In general, with very few exceptions, actuarial approaches are as accurate or more accurate than "expert" judgment [@Dawes1989; @Aegisdottir2006; @Baird2000; @DenHartigh2018; @Grove1996; @Grove2000].
...
When operating freely, clinicians tend to over-estimate exceptions to the established rules (i.e., the broken leg syndrome).
...
Because actuarial methods are based on actual population levels (i.e., [base rates](#sec-baseRates)), unique exceptions are not over-estimated.
...
The best actuarial models tend to be relatively simple (parsimonious), that can account for one or several of the most important predictors and their optimal weightings, and that account for the base rate of the phenomenon.
