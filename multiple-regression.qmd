...
By including multiple [predictor variables](#sec-correlationalStudy) in prediction of the [outcome variable](#sec-correlationalStudy), it also allows improved prediction accuracy.

In multiple regression, we estimate models.
A model is a simplification of reality.
All statistical analyses follow the same basic structure, as in @eq-analysisBasicStructure [@Brauer2018]:

$$
\text{DATA} = \text{MODEL} + \text{ERROR}
$$ {#eq-analysisBasicStructure}
...
That is, multiple regression identifies the optimal linear combination of the [predictor variables](#sec-correlationalStudy) that explains the most variance in the [outcome variable](#sec-correlationalStudy).
The intercept is the expected value of the [outcome variable](#sec-correlationalStudy) when all of the [predictor variables](#sec-correlationalStudy) have a value of zero.
...
### Standardized Regression Coefficient {#sec-multipleRegressionComponentsStandardizedCoefficient}

The standardized regression coefficient is computed as in @eq-standardizedRegressionCoefficient:

$$
\small
\text{standardized regression coefficient} = \text{unstandardized regression coefficient} \cdot \frac{\text{standard deviation of }X}{\text{standard deviation of }Y}
$$ {#eq-standardizedRegressionCoefficient}
...
The standardized regression coefficient ($\beta$) is interpreted such that, for every standard deviation change in the [predictor variable](#sec-correlationalStudy), there is a __ standard deviation change in the [outcome variable](#sec-correlationalStudy) (holding all other [predictor variables](#sec-correlationalStudy) constant).
...
In this way, standardized regression coefficients provide a meaningful index of [effect size](#sec-practicalSignificance) and can be used to identify the [predictors](#sec-correlationalStudy) with the strongest [predictive validity](#sec-predictiveValidity).

### Standard Error {#sec-multipleRegressionComponentsStandardError}
...
### Confidence Interval {#sec-multipleRegressionComponentsConfidenceInterval}

A *confidence interval* represents a range of plausible values for a parameter, constructed so that, with repeated sampling, the intervals contain the true value with a specified long-run frequency (e.g., 95%).
...
The 95% confidence interval is computed as the parameter estimate plus or minus 1.96 standard errors (because in a standard normal distribution, the middle 95% of the distribution lies between −1.96 and +1.96).
...
That is, if we used the same sampling procedure repeatedly, about 95% of the confidence intervals (computed as parameter estimate $\pm `r qnorm(.975)` \ \text{standard errors}$) would contain the true value of the regression coefficient.

## Types of Regression {#sec-multipleRegressionTypes}

When the [outcome variable](#sec-correlationalStudy) is continuous or semicontinuous, [linear regression](#sec-multipleRegressionLinear) is common.
...
We fit a logistic regression model using the `stats::glm()` function and specifying `family = binomial()`.
To calculate the model $R^2$, we use the `performance::r2()` function of the `performance` package [@R-performance; @Ludecke2021_packages].
...
We fit an ordinal regression model using the `ordinal::clm()` function of the `ordinal` package [@R-ordinal].
...
We fit a negative binomial regression model using the `MASS::glm.nb()` function of the `MASS` package [@R-MASS].
...
We fit a (Bayesian) zero-one-inflated beta regression model (to allow for zeros and ones) using the `brms::brm()` function of the `brms` package [@R-brms] and specifying `family = zero_one_inflated_beta()`.
...
load(file = "./data/betaRegression.RData") 
```
...
petersenlab::robust_load("https://osf.io/download/fe37j/") #load(url("https://osf.io/download/fe37j/"))
```
...
If the residuals differ as a function of the [outcome](#sec-correlationalStudy) or [predictor](#sec-correlationalStudy) variable, this is called heteroscedasticity.
...
Examples of linear and nonlinear associations are depicted with [scatterplots](#sec-scatterplot) in @fig-linearNonlinearAssociationsPDF.

```{r}
#| label: fig-linearNonlinearAssociationsPDF
#| fig-cap: "Example Associations Depicted With Scatterplots."
#| fig-alt: "Example Associations Depicted With Scatterplots."
#| code-fold: true
...
QQ plots depict quantiles of a sample distribution (y-axis) compared to the quantiles of a theoretical (in this case, normal) distribution (x-axis).
PP plots depict cumulative probabilities of a sample distribution (y-axis) compared to those of a theoretical (in this case, normal) distribution (x-axis).
...
Various examples of QQ plots and deviations from normality are depicted in @fig-distributionQQplotsPDF.

```{r}
#| label: fig-distributionQQplotsPDF
#| fig-cap: "Quantile–Quantile (QQ) Plots of Various Distributions (Right Side) with the Histogram of the Associated Distribution (Left Side)."
#| fig-alt: "Quantile–Quantile (QQ) Plots of Various Distributions (Right Side) with the Histogram of the Associated Distribution (Left Side)."
#| code-fold: true
...
The coefficient of determination ($R^2$) reflects the proportion of variance in the [outcome (dependent) variable](#sec-correlationalStudy) that is explained by the model predictions (i.e., by the optimal linear combination of the [predictor variable(s)](#sec-correlationalStudy)), as in @eq-coefficientOfDeterminationPartitionedVariance: $R^2 = \frac{\text{variance explained in }Y}{\text{total variance in }Y}$.
...
## How Much Variance Each Predictor Explains {#sec-multipleRegressionExplainedVariancePredictors}

Another estimate of [effect size](#sec-practicalSignificance) in addition to [standardized regression coefficients](#sec-multipleRegressionComponentsStandardizedCoefficient) is the squared semi-partial correlation ($sr^2$).
The squared semi-partial correlation reflects the proportion of variance in the [outcome variable](#sec-correlationalStudy) that is uniquely explained by each [predictor variable](#sec-correlationalStudy).
The total model $R^2$ minus the sum of all the squared semi-partial correlations reflects the proportion of variance in the [outcome variable](#sec-correlationalStudy) that is explained by the common variance among the [predictor variables](#sec-correlationalStudy).
...
Overfitting is most likely to occur when the model is too complex relative to the sample size (e.g., many [predictor variables](#sec-correlationalStudy) or parameters)
...
The regression coefficients (and resulting accuracy) tend to become weaker when applied to new data, a phenomenon called [shrinkage](#sec-shrinkage), which is described in @sec-shrinkage.
For instance, shrinkage is observed when applying multiple regression models to new data in @sec-fittingModels-regression and when applying [machine learning](#sec-machineLearning) models to new data in @sec-fittingModels-machineLearning.
...
However, it is worth noting that, for several reasons, including a variable as a covariate in a model does not necessarily fully control for that construct.
First, your measure of the covariate has measurement error and may not fully capture the construct, which can lead to [residual confounding](#sec-causalDiagramConfounding).
Second, including the covariate only accounts for the linear effect of that variable.
The variable (e.g., age) could have nonlinear effects that are not captured by the linear term (and may require nonlinear terms, e.g., quadratic).
Third, the construct could interact with other constructs, which would also not be captured by the linear term (and may require interaction terms).
Thus, it is important to consider the reliability of the covariate, and the ways in which the covariate is related to the outcome including any nonlinearity and interactions.

In general, you should select which covariates to include based on theory and your [causal diagram](#sec-causalDiagrams) of the causal process (as described in @sec-causalDiagrams), not based on whether the covariate has a [statistically significant](#sec-statisticalSignificance) association with the [outcome variable](#sec-correlationalStudy).
Just because a [predictor variable](#sec-correlationalStudy) is not associated with an [outcome variable](#sec-correlationalStudy) does not mean that the [predictor variable](#sec-correlationalStudy) will not be useful in the model.
For instance, the [predictor variable](#sec-correlationalStudy) could have a [suppression effect](#sec-multipleRegressionSuppression), or a [moderating effect](#sec-moderation), etc., that may not be detectable based on a bivariate association.
...
Let's first examine [descriptive statistics](#sec-descriptiveStatistics) of the [predictor](#sec-correlationalStudy) and [outcome](#sec-correlationalStudy) variables.
...
Then, let's examine the bivariate association of each using a [scatterplot](#sec-scatterplot) to evaluate for any potential nonlinearity, as depicted in @fig-scatterplotsPDF.

```{r}
#| label: fig-scatterplotsPDF
#| fig-cap: "Scatterplots With Fantasy Points (Season) Among Wide Receivers. The linear best-fit line is in black. The nonlinear best-fit line is in blue."
#| fig-alt: "Scatterplots With Fantasy Points (Season) Among Wide Receivers. The linear best-fit line is in black. The nonlinear best-fit line is in blue."
...
If we want to obtain standardized regression coefficients, we can use the `effectsize::standardize_parameters()` function of the `effectsize` package [@R-effectsize; @BenShachar2020_packages].

```{r}
print(effectsize::standardize_parameters(linearRegressionModel, method = "basic"), digits = 2)
```

Or, we can standardize the [outcome variable](#sec-correlationalStudy) and each [predictor variable](#sec-correlationalStudy) using the `base::scale()` function:
...
### Squared Semi-Partial Correlation ($sr^2$) {#sec-multipleRegressionSquaredSemiPartialCorrelation}

To estimate the [squared semi-partial correlation](#sec-multipleRegressionExplainedVariancePredictors) ($sr^2$), we use the `rockchalk::getDeltaRsquare()` function from the `rockchalk` package [@R-rockchalk].

```{r}
rockchalk::getDeltaRsquare(linearRegressionModel)
```

### Commonality Analysis {#sec-multipleRegressionCommonalityAnalysis}

Commonality analysis decomposes the model's $R^2$ into coefficients that represent the proportion of variance accounted for uniquely by each [predictor variable](#sec-correlationalStudy) and the proportion of variance accounted for that is shared in each possible combination of the [predictor variables](#sec-correlationalStudy).
Below we conduct a commonality analysis using the `yhat::calc.yhat()`, `yhat::commonality()`, and `yhat::commonalityCoefficients()` functions of the `yhat` package [@R-yhat].
...
A [standardized regression coefficient](#sec-multipleRegressionComponentsStandardizedCoefficient) ($\beta$) may not be the most be the best way of evaluating the importance of a [predictor variable](#sec-correlationalStudy).
As described in @sec-multipleRegressionComponentsStandardizedCoefficient, a [standardized regression coefficient](#sec-multipleRegressionComponentsStandardizedCoefficient) ($\beta$) is interpreted such that, for every standard deviation change in the [predictor variable](#sec-correlationalStudy), there is a __ standard deviation change in the [outcome variable](#sec-correlationalStudy), holding all other [predictor variables](#sec-correlationalStudy) constant.
However, the [predictor variables](#sec-correlationalStudy) are often correlated.
Thus, it may not be an accurate reflection of the set of [predictor variables](#sec-correlationalStudy) to hold all other [predictor variables](#sec-correlationalStudy) constant, when they are correlated in reality.
As one [predictor variable](#sec-correlationalStudy) moves, it may be likely that other [predictor variables](#sec-correlationalStudy) move, as well.
Moreover, the [predictor](#sec-correlationalStudy) with the largest beta (i.e., [standardized regression coefficient](#sec-multipleRegressionComponentsStandardizedCoefficient)) does not necessarily contribute the most to the proportion of variance explained—i.e., to the [coefficient of determination](#sec-multipleRegressionRSquared) ($R^2$).

We can perform a dominance analysis to evaluate the relative importance of the [predictors](#sec-correlationalStudy) in the regression model.
To perform the dominance analysis, we use the `parameters::dominance_analysis()` function of the `parameters` package [@R-parameters; @Luedecke2020_packages], which leverages the `domir` package [@R-domir].

```{r}
parameters::dominance_analysis(linearRegressionModel)
```

We can also conduct a dominance analysis using the `yhat::dominance()` function of the `yhat` package [@R-yhat].
...
To visualize the regression coefficients, we can use the `broom::tidy()` function of the `broom` package [@R-broom], as in Figures [-@fig-regressionCoefficients] and [-@fig-regressionCoefficientsStandardized].
...
We can calculate this by substituting in the regression coefficients and [predictor](#sec-correlationalStudy) values.
...
\begin{aligned}
  ...
  &`r petersenlab::apa(beta5, decimals = 2)` \cdot `r petersenlab::apa(weight, decimals = 0)` + `r petersenlab::apa(beta6, decimals = 2)` \cdot `r petersenlab::apa(weight, decimals = 0)`^2 + `r petersenlab::apa(beta7, decimals = 2)` \cdot `r petersenlab::apa(target_share, decimals = 1)` + `r petersenlab::apa(beta8, decimals = 2)` \cdot `r petersenlab::apa(target_share, decimals = 1)`^2 + \epsilon \\
...
We can apply a transformation to the [outcome variable](#sec-correlationalStudy) to generate a more normally distributed variable using the `bestNormalize::bestNormalize()` function of the `bestNormalize` package [@R-bestNormalize; @Peterson2021_packages; @Peterson2020_packages].
...
## Suppression {#sec-multipleRegressionSuppression}

Suppression in a regression model occurs when including a [predictor variable](#sec-correlationalStudy) increases the magnitude of the regression coefficient (i.e., beta) of another [predictor variable](#sec-correlationalStudy) on an [outcome variable](#sec-correlationalStudy) [@MacKinnon2000].
The suppressor variable suppresses irrelevant variance in another [predictor variable](#sec-correlationalStudy), allowing the [predictor variable's](#sec-correlationalStudy) association with the [outcome variable](#sec-correlationalStudy) to emerge more clearly.
Three types of suppression are depicted in @GaylordHarden2010.
...
Multiple regression in `R` using the `stats::lm()` function applies listwise deletion.
*Listwise deletion* (also called complete case analysis) removes any row (in the data file) from analysis that has a missing value on the [outcome variable](#sec-correlationalStudy) or any of the [predictor variables](#sec-correlationalStudy).
...
Pairwise deletion (also called available case analysis) uses all available data for each pair of variables when estimating the correlations/covariances, and the resulting correlation/covariance matrix can be used to estimate a multiple regression model.
...
Multiple imputation takes a data set with missingness and it uses available information on other variables to estimate what likely values could have been for the missing value.
It repeats the imputation multiple times, so there are multiple imputed data sets, which can give a sense of the degree of uncertainty of each imputed value.
The model is then fit to each of the multiply imputed data sets separately, and the results are combined.
...
- NOT a [predictor](#sec-correlationalStudy) of the outcome: `0`
- [cluster variable](#sec-mixedModelDataStructure): `-2`
- [fixed effect](#sec-mixedModelsOverview) of [predictor](#sec-correlationalStudy): `1`
- [fixed effect](#sec-mixedModelsOverview) and [random effect](#sec-mixedModelsOverview) of [predictor](#sec-correlationalStudy): `2`
- include cluster mean of [predictor](#sec-correlationalStudy) in addition to [fixed effect](#sec-mixedModelsOverview) of [predictor](#sec-correlationalStudy): `3`
- include cluster mean of [predictor](#sec-correlationalStudy) in addition to [fixed effect](#sec-mixedModelsOverview) and [random effect](#sec-mixedModelsOverview) of [predictor](#sec-correlationalStudy): `4`
...
Full information maximum likelihood (FIML) estimates model parameters using all available data for each case, without imputing missing values.
FIML is commonly used for handling missingness in approaches to latent variable modeling, such as [factor analysis](#sec-factorAnalysis) and structural equation modeling.
...
## Impact of Outliers and Restricted Range {#sec-multipleRegressionOutliers}

[As with correlation](#sec-correlation), multiple regression can be strongly impacted by [outliers](#sec-correlationOutliers) and [restricted range](#sec-correlationRestrictedRange).
...
One approach is to use an MM-type estimator, such as is used in the `robustbase::lmrob()` and `robustbase::glmrob()` functions of the `robustbase` package [@R-robustbase; @Todorov2009_packages].
...
We set up the bootstrap folds using the `rsample::bootstraps()` function of the `rsample` package [@R-rsample].
...
We can also estimate a model with multiple hypothesized [mediators](#sec-mediation):
