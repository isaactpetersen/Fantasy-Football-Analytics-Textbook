...
We can also evaluate residual plots (@fig-residualPlots), marginal model plots (@fig-marginalModelPlots), and added-variable plots (@fig-addedVariablePlots), to evaluate for potential nonlinearity.
...
To do this, we can estimate the intraclass correlation coefficient (ICC) from a [mixed model](#sec-mixedModels).
...
Now, let's specify the imputation method:
