...
Using the data, the best fitting regression model is: $y = \text{`{r} regression1_intercept`} + \text{`{r} regression1_slope`} \cdot x_1$.
In this example, the $R^2$ is $\text{`{r} regression1_rsquare`}$.
...
With the second [predictor variable](#sec-correlationalStudy), the best fitting regression model is: $y = \text{`{r} regression2_intercept`} + \text{`{r} regression2_slope1`} \cdot x_1 + \text{`{r} regression2_slope2`} \cdot x_2$.
In this example, the $R^2$ is $\text{`{r} regression2_rsquare`}$.
...
#| fig-cap: "Over-fitting Model in Red Relative to the True Distribution of the Data in Blue. From @PetersenPrinciplesPsychAssessment."
#| echo: false
#| fig.height: 8
#| fig.width: 8
