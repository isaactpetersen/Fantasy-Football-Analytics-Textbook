...
If we want to obtain standardized regression coefficients, we can use the `lm.beta()` function of the `QuantPsyc` package.

```{r}
QuantPsyc::lm.beta(linearRegressionModel)
```
...
```{r}
QuantPsyc::lm.beta(quadraticTermsRegressionModel)
```
...
To visualize the regression coefficients, we can use the `tidy()` function of the `broom` package.

```{r}
quadraticTermsRegressionModel_tidy <- tidy(
  quadraticTermsRegressionModel,
  conf.int = TRUE)

quadraticTermsRegressionModelStandardized_tidy <- tidy(
