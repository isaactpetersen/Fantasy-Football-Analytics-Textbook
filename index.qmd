...
knitr::opts_chunk$set(
  time = TRUE,
  message = NA # Show in console, not in rendered output
)
...
# Log file setup
log_file <- "chunk-times.txt"
if (file.exists(log_file)) file.remove(log_file)

# Environment to track start times
.chunk_times_env <- new.env()

# Timing hook
knitr::knit_hooks$set(time = function(before, options) {
  label <- options$label %||% "<unnamed>"
  
  if (before) {
    .chunk_times_env[[label]] <- Sys.time()
  } else {
    start_time <- .chunk_times_env[[label]]
    if (!is.null(start_time)) {
      elapsed <- difftime(Sys.time(), start_time, units = "secs")
      msg <- sprintf("Chunk `%s` ran in %.2f seconds.", label, as.numeric(elapsed))
      
      # Send message to GitHub Actions log
      message(sprintf("::notice title=Chunk Time::%s", msg))
...
packagesForBibliography <- c(
  ...
  "knitr", "rmarkdown", "dplyr", "ggplot2", "tidyverse", "broom", "broom.mixed", 
  "downlit", "xml2", "nflreadr", "nflplotR", "nflfastR", "nflseedR", "nfl4th", 
  "gsisdecoder", "progressr", "DescTools", "pwr", "pwrss", "WebPower", "XICOR", 
  "dagitty", "ggdag", "lubridate", "ggtext", "gghighlight", "ggExtra", "grid", 
  "patchwork", "pROC",  "lme4", "lmerTest", "MuMIn", "emmeans", "pbkrtest", 
  "sjstats", "AICcmodavg", "rstan", "brms", "tidybayes", "bbmle", 
  "fitdistrplus", "sn", "mclust", "magrittr", "viridis", "plotly", "quantmod", 
  "fPortfolio", "NMOF", "nFactors", "xts", "zoo", "forecast", "stringi", #"webshot2"
  "parallel", "parallelly", "doParallel", "missRanger", "ggridges", "powerjoin", 
  "caret", "tidymodels", "LongituRF", "gpboost", "cmdstanr", "learnr", 
  "remotes", "corrplot", "correlation", "mgcv", "rms", "car", "performance", 
  "lavaan", "lavaangui", "mice", "miceadds", "interactions", "shiny", 
  "PearsonDS", "robustbase", "ordinal", "MASS", "data.table", "future", 
  "future.apply", "QuantPsyc", "SimDesign", "effectsize"
)
...
In particular, Andrew Tungate made important additions to the [`ffanalytics` `R` package](https://github.com/FantasyFootballAnalytics/ffanalytics) [@R-ffanalytics] that were necessary for conducting a [bootstrap simulation](#sec-simulationProjectedPointsBootstrapping) of players' projected fantasy points.
