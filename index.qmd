...
# Enable automatic chunk timing logging to GitHub Actions
knitr::opts_chunk$set(time = TRUE) # Apply to ALL chunks globally
...
knitr::knit_hooks$set(time = function(before, options) {
  if (before) {
    options$start_time <- Sys.time()
  } else {
    elapsed <- difftime(Sys.time(), options$start_time, units = "secs")
    label <- options$label %||% "<unnamed>"
    msg <- sprintf("Chunk `%s` ran in %.2f seconds.", label, as.numeric(elapsed))

    # Log to GitHub Actions via stdout (so it's not suppressed)
    cat(sprintf("::notice title=Chunk Time::%s\n", msg))
...
packagesForBibliography <- c(
  ...
  "fPortfolio", "NMOF", "xts", "zoo", "forecast", "stringi", "parallel", #"webshot2"
  "parallelly", "doParallel", "missRanger", "ggridges", "powerjoin", "caret", 
  "tidymodels", "LongituRF", "gpboost", "cmdstanr", "learnr", "remotes", 
  "corrplot", "correlation", "mgcv", "rms", "car", "performance", "lavaan", 
  "mice", "miceadds", "interactions", "shiny", "PearsonDS", "robustbase", 
  "ordinal", "MASS", "data.table", "future", "future.apply", "QuantPsyc", 
  "SimDesign", "effectsize"
)
