...
The mean of $x$ (written as: $\bar{x}$ or $\mu_x$) is calculated as in @eq-mean:
...
For instance, the mean of the values of 0, 0, 10, 15, 20, 30, and 1000 is `{r} petersenlab::apa(exampleValues_mean, decimals = 2)`.
Another issue with the mean is that it may not be the same as any individual's value [@Rose2016]—that is, it may be unrepresentative of all of the individuals whose values comprise the average.
...
The pseudomedian can be computed using the `DescTools::HodgesLehmann()` function of the `DescTools` package [@R-DescTools].
The pseudomedian of the values of 0, 0, 10, 15, 20, 30, and 1000 is `{r} exampleValues_pseudomedian`.
The pseudomedian is an example of a robust average—an index of the center of a distribution that is less sensitive to outliers.
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `petersenlab::Mode()` function for computing the mode of a set of data.
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains various functions for computing the weighted median (i.e., a weighted quantile at the 0.5 quantile, which is equivalent to the 50th percentile) based on @Akinshin2023.
...
The ([sample](#sec-sampleVsPopulation)) variance of $x$ (written as: $s^2$) is calculated as in @eq-variance:
...
The [population](#sec-sampleVsPopulation) variance divides by $n$ rather than $n-1$ and is written as $\sigma^2$.

The ([sample](#sec-sampleVsPopulation)) standard deviation of $x$ (written as: $s$) is calculated as in @eq-sd:

$$
s = \sqrt{\frac{\sum (x_i - \bar{x})^2}{n-1}}
$$ {#eq-sd}

The [population](#sec-sampleVsPopulation) standard deviation divides by $n$ rather than $n-1$ and is written as $\sigma$.
...
The coefficient of variation (CV) accounts for the fact that variables with larger means also tend to have larger standard deviations; it is calculated as the standard deviation divided by the mean, as in @eq-coefficientOfVariation.

If you want to give some values more weight to others, you can calculate weighted quantiles, while assigning a weight to each value.
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains various functions for computing weighted quantiles based on @Akinshin2023.
Because some projections are outliers, we use a trimmed version of the weighted Harrell-Davis quantile estimator for greater robustness.
...
```{r}
sd(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- variance (`stats::var()`):

```{r}
var(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- range (`base::range()`):

```{r}
range(player_stats_seasonal$fantasyPoints, na.rm = TRUE)

max(player_stats_seasonal$fantasyPoints, na.rm = TRUE) - min(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- minimum (`base::min()`) and maximum (`base::max()`):

```{r}
min(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
max(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- quantiles (in this case, the 10th and 90th quantiles; `stats::quantile()`):

```{r}
quantile(player_stats_seasonal$fantasyPoints, c(0.10, 0.90), na.rm = TRUE)
```
...
```{r}
IQR(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- median absolute deviation (`stats::mad()`):

```{r}
mad(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- coefficient of variation:

```{r}
sd(player_stats_seasonal$fantasyPoints, na.rm = TRUE) / mean(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```
...
The skewness represents the degree of asymmetry of a distribution around its mean.
...
Positive kurtosis values reflect a leptokurtic distribution, where there are heavier tails and a sharper peak than a normal distribution, indicating more frequent extreme values.
Negative kurtosis values reflect a platykurtic distribution, where there are lighter tails and a flatter peak than a normal distribution, indicating fewer extreme values.
Fantasy points tend to have a leptokurtic distribution.

Below is R code to estimate each using the `psych::skew()` and `psych::kurtosis()` functions of the `psych` package [@R-psych]:

- skewness (`psych::skew()`):

```{r}
psych::skew(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```

- kurtosis (`psych::kurtosi()`):

```{r}
psych::kurtosi(player_stats_seasonal$fantasyPoints, na.rm = TRUE)
```
...
### Interactive Visualization {#sec-descriptiveStatisticsVisualization}

You can use the following interactive graph to visualize various distributions according to a specified [center](#sec-descriptiveStatisticsCenter) (mean), [spread](#sec-descriptiveStatisticsSpread) (standard deviation), and [shape](#sec-descriptiveStatisticsShape) (skewness and kurtosis).
Try varying the standard deviation, skewness, and kurtosis to see the impact of each.
The interactive graph was created using the `shiny` [@R-shiny] and `PearsonDS` [@R-PearsonDS] packages.
The source code for interactive graph is here: <https://github.com/isaactpetersen/DistributionVisualizer>.

```{=html}
<iframe id="distributionVisualizer" src="https://ffashiny.shinyapps.io/distributionvisualizer/" style="border: none; width: 100%; height: 1000px" frameborder="0"></iframe>
```
...
If you want to determine the *z* score associated with a particular percentile in a normal distribution, you can use the `stats::qnorm()` function.
...
Statistical significance is the reliability (i.e., consistency) of an effect, and it is evaluated with the *p*-value.
...
For a test of group differences, the *p*-value evaluates the likelihood that you would observe a difference as large or larger than the one you observed between the groups if there were no systematic difference between the groups in the population, as depicted in @fig-pValuesDifferencePDF.

```{r}
#| label: fig-pValuesDifferencePDF
#| fig-cap: "Interpretation of *p*-Values When Examining The Differences Between Groups. The vertical black lines reflect the group means."
#| fig-alt: "Interpretation of *p*-Values When Examining The Differences Between Groups. The vertical black lines reflect the group means."
#| code-fold: true
...
For a test of whether two variables are associated, the *p*-value evaluates the likelihood that you would observe an association as strong or stronger than the one you observed if there were no actual association between the variables in the population, as depicted in @fig-pValuesAssociationPDF.

```{r}
#| label: fig-pValuesAssociationPDF
#| fig-cap: "Interpretation of *p*-Values When Examining The Association Between Variables."
#| fig-alt: "Interpretation of *p*-Values When Examining The Association Between Variables."
#| code-fold: true
...
A plot of statistical power is in @fig-nhst, as adapted from @Magnusson2013 (archived at <https://perma.cc/FG3J-85L6>).
...
ggplot(
  ...
  scale_fill_manual(
    "test",
    values = c("alpha" = "#0d6374", "beta" = "#be805e", "power"="#7cecee")) +
  # beta arrow
  annotate(
    "segment",
    x = 0.1,
...
- <https://rpsychologist.com/pvalue> [@Magnusson2021\; archived at <https://perma.cc/JP9F-9ZVY>]
- <https://rpsychologist.com/d3/pdist> [@Magnusson2015\; archived at <https://perma.cc/BE96-8LSJ>]
- <https://rpsychologist.com/d3/nhst> [@Magnusson2014\; archived at <https://perma.cc/ZU9A-37F3>]
...
Cohen's $d$ is commonly used for evaluating the magnitude of group differences.
...
The standardized regression coefficient is interpreted such that, for every 1 standard deviation increase in $x$, there was a _ standard deviation [increase/decrease] in $y$.

#### Correlation Coefficient ($r$)

The formula for the correlation coefficient is in @eq-pearsonCorrelation.
...
where $y$ is the [outcome variable](#sec-correlationalStudy), $y_i$ is the observed value of the [outcome variable](#sec-correlationalStudy) for the $i$th observation, $\hat{y}_i$ is the model predicted value for the $i$th observation, and $\bar{y}$ is the mean of the observed values of the [outcome variable](#sec-correlationalStudy).
The total sum of squares is an index of the total variation in the [outcome variable](#sec-correlationalStudy).

The coefficient of determination typically ranges from 0 to 1, but it can be negative.
A negative coefficient of determination indicates that the predicted values are less accurate than using the mean of the actual values as the predicted values (i.e., predicting the mean of the observed values for every case).

#### Squared Semi-Partial Correlation ($sr^2$) {#sec-squaredSemiPartialCorrelation}

Squared semi-partial correlations are described in @sec-multipleRegressionExplainedVariancePredictors.
...
| Squared semi-partial correlation ($sr^2$)           | $\ge .01$   | $\ge .06$   | $\ge .14$   |
...
Indeed, [*t*-test](#sec-tTest), [ANOVA](#sec-anova), and [regression](#sec-regression) are all special cases of the general linear model, which in turn, is a special case of [mixed models](#sec-mixedModels) without [random effects](#sec-mixedModelsOverview) [@Brauer2018].

@JacksonWood2017 provides an online, interactive statistical decision tree to help you decide which statistical analysis to use: <https://www.statsflowchart.co.uk>
...
The *F* statistic (which is used to determine statistical significance) in ANOVA is computed by first partitioning the sum of squares as in @eq-partitionSumSquares:

$$
SS_{\text{total}} = SS_{\text{between}} + SS_{\text{within}}
$$ {#eq-partitionSumSquares}

where $SS_{\text{total}}$ is the total sum of squares, which is equal to @eq-sumSquaresTotal:

$$
SS_{\text{total}} = \sum_{i = 1}^{k} \sum_{j = 1}^{n_i} \left( X_{ij} - \bar{X}_{..} \right)^2
$$ {#eq-sumSquaresTotal}

where $X_{ij}$ is the raw data point (i.e., observation $j$ within group $i$), $n_i$ is the sample size of group $i$, $\bar{X}_{i.}$ is the mean of group $i$, and $\bar{X}_{..}$ is the grand mean (the mean across all observations from all groups).

$SS_{\text{between}}$ is the between-groups sum of squares, which is equal to @eq-sumSquaresBetween:

$$
SS_{\text{between}} = \sum_{i = 1}^{k} n_i \left( \bar{X}_{i.} - \bar{X}_{..} \right)^2
$$ {#eq-sumSquaresBetween}

and $SS_{\text{within}}$ is the within-groups sum of squares, which is equal to @eq-sumSquaresWithin:

$$
SS_{\text{within}} = \sum_{i = 1}^{k} \sum_{j = 1}^{n_i} \left( X_{ij} - \bar{X}_{i.} \right)^2
$$ {#eq-sumSquaresWithin}

Mean squares are then computed from the sum of squares as in @eq-meanSquares:
...
where $df_{\text{between}}$ is the between-groups degrees of freedom, $df_{\text{within}}$ is the within-groups degrees of freedom, $k$ is the number of groups, and $N$ is the sample size.

Then, the *F* statistic is computed as the ratio of between-groups mean squares to within-groups mean squares as in @eq-fStatistic:

$$
F = \frac{MS_{\text{between}}}{MS_{\text{within}}}
$$ {#eq-fStatistic}

In sum, ANOVA examines the ratio of between-groups sum of squares to within-groups sum of squares (after adjusting for degrees of freedom).
...
However, unlike a *t*-test, ANOVA allows examining multiple [independent variables](#sec-experiment) and more than two groups.
...
ANOVA has key limitations.
ANOVA cannot handle continuous [independent](#sec-experiment) or [predictor](#sec-correlationalStudy) variables that vary within units [@Brauer2018].
In addition, ANOVA poorly handles missing data—it uses listwise deletion and thus excludes participants with missing values on any of the [independent](#sec-experiment) or [dependent](#sec-experiment) variables [@Brauer2018].
...
where $x$ is the [predictor variable](#sec-correlationalStudy), $y$ is the [outcome variable](#sec-correlationalStudy), $\text{Cov}(x, y)$ is the covariance between $x$ and $y$, $s_x$ is the standard deviation of $x$, $s_y$ is the standard deviation of $y$, $n$ is the sample size, $z_x$ is the [*z* scores](#sec-zScores) for $x$, and $z_y$ is the [*z* scores](#sec-zScores) for $y$.
...
- [alpha level](#sec-statisticalSignificance) (Type I error rate)

Knowing any three of the following, you can calculate the fourth (assuming the assumptions of the statistical test are met): statistical power, [effect size](#sec-practicalSignificance), [sample size](#sec-sampleVsPopulation), and [alpha level](#sec-statisticalSignificance) (Type I error rate).
Below is `R` code for calculating power for each of various statistical tests (i.e., a *power analysis*).
We use the `pwr` [@R-pwr] and `pwrss` [@R-pwrss] packages to estimate statistical power.
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.t.test()`):
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size per group, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.t.test()`):
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size per group, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.t2n.test()`):
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size per group, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.anova.test()`):
...
Solving for sample size per group needed (given [effect size](#sec-practicalSignificance), power, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.anova.test()`):
...
Solving for the minimum detectable [effect size](#sec-practicalSignificance) (given sample size per group, power, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.anova.test()`):
...
##### Simulation {#sec-statisticalPowerOneWayANOVAsimulation}
...
Below is an example of evaluating the statistical power for detecting an effect unbalanced designs via simulation using the `SimDesign` [@R-SimDesign; @Chalmers2020_packages] package.
We discuss simulation more in @sec-simulation.

###### 1. Design Matrix {#sec-statisticalPowerOneWayANOVAsimulationDesign}
...
###### 2. Generate {#sec-statisticalPowerOneWayANOVAsimulationGenerate}
...
###### 3. Analyse {#sec-statisticalPowerOneWayANOVAsimulationAnalyse}
...
###### 4. Summarise {#sec-statisticalPowerOneWayANOVAsimulationSummarize}
...
###### 5. Run the Simulation {#sec-statisticalPowerOneWayANOVAsimulationRun}
...
###### Simulation Results {#sec-statisticalPowerOneWayANOVAsimulationResults}
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size per group, and [alpha level](#sec-statisticalSignificance); `WebPower::wp.rmanova()`):
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size per group, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.r.test()`):
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.f2.test()`):
...
Solving for statistical power achieved (given [effect size](#sec-practicalSignificance), sample size, and [alpha level](#sec-statisticalSignificance); `pwr::pwr.chisq.test()`):
...
- <https://aguinis.shinyapps.io/ml_power> [@Mathieu2012]
- <https://koumurayama.shinyapps.io/summary_statistics_based_power> [@Murayama2022]
- <https://webpower.psychstat.org/wiki/models/index> [@Zhang2018]
...
- <https://sjak.shinyapps.io/power4SEM> [@Jak2020]
- <https://sempower.shinyapps.io/sempower> [@Moshagen2024]
- <https://yilinandrewang.shinyapps.io/pwrSEM> [@Wang2021]
- <https://webpower.psychstat.org/wiki/models/index> [@Zhang2018]

#### Mediation and Moderation {#sec-statisticalPowerMediationModeration}

There are free tools for calculating power for tests of [mediation](#sec-mediation) and [moderation](#sec-moderation):

- <https://schoemanna.shinyapps.io/mc_power_med> [@Schoemann2017]
- <https://www.causalevaluation.org/power-analysis.html> (web application: <https://powerupr.shinyapps.io/index/>) [@Ataneka2023], based on the `PowerUpR` package [@Bulus2021]
- <https://webpower.psychstat.org/wiki/models/index> [@Zhang2018]

## Ethics of Reporting Statistics {#sec-ethicsReportingStatistics}

It is important to report statistics ethically and responsibly.
People can use statistics to lie [@Huff2023].
It is important to present statistics fairly and accurately even if they are not convenient to your preferred narrative.
In addition, it is important to consider [practical significance](#sec-practicalSignificance) in addition to [statistical significance](#sec-statisticalSignificance).
...
Null hypothesis significance testing is a framework for [inferential statistics](#sec-inferentialStatistics), in which there is a [null hypothesis](#sec-nullHypothesis) and [alternative hypothesis](#sec-alternativeHypothesis).
