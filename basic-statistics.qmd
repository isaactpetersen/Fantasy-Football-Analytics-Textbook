...
The pseudomedian can be computed using the `DescTools` package [@R-DescTools].
...
The [`petersenlab`](https://cran.r-project.org/web/packages/petersenlab/index.html) package [@R-petersenlab] contains the `Mode()` function for computing the mode of a set of data.
...
- quantiles (in this case, the 10th and 90th quantiles):

```{r}
quantile(player_stats_seasonal$fantasyPoints, c(0.10, 0.90), na.rm = TRUE)
```

- weighted quantiles (in this case, the 10th and 90th quantiles):
