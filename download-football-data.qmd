...
Below, we provide examples for how to download various types of NFL data using the `nflreadr` package [@R-nflreadr].
...
If you have difficulty downloading the data files using the `nflreadr` package [@R-nflreadr], we also saved the data files so they are publicly available on the Open Science Framework: <https://osf.io/z6pg4>.
...
For instance, here are the first 6 players by ID according to `gsis_id`:
...
Note how it is more clear, at a glance and without additional, who these players are.
...
First, we can compute the players' season-by-season statistics.
...
First, we can compute the players' career statistics using the `calculate_player_stats()`, `calculate_player_stats_def()`, and `calculate_player_stats_kicking()` functions from the `nflfastR` package [@R-nflfastR] for offensive players, defensive players, and kickers, respectively.
