...
The `players_projections_seasonal_raw` object is in `player`-`position`-`projection source` form.
...
```{r}
players_projectedStats_seasonal <- ffanalytics::projections_table(
  players_projections_seasonal_raw,
  scoring_rules = scoring_obj,
  return_raw_stats = TRUE)

players_projectedPoints_seasonal <- ffanalytics::projections_table(
  players_projections_seasonal_raw,
  scoring_rules = scoring_obj,
  return_raw_stats = FALSE)
```

The `players_projectedStats_seasonal` and `players_projectedPoints_seasonal` objects are in `player`-`average type`-`position` form.
...
players_projectedStats_seasonal %>% 
  group_by(id, avg_type, position) %>% 
  filter(n() > 1) %>% 
  head()
```

Let's merge the two objects.
