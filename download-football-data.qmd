...
We calculate historical actual season-by-season statistics in @sec-calculateActualSeasonalStats.
...
```{r}
names(nfl_actualStats_offense_weekly) %>% as.data.frame()

names(nfl_actualStats_defense_weekly) %>% as.data.frame()

names(nfl_actualStats_kicking_weekly) %>% as.data.frame()
```
...
First, we can compute the players' season-by-season statistics using the `nflfastR` [@R-nflfastR] package.
...
We already load players' week-by-week statistics [above](#sec-downloadActualStats) using the `nflreadr::load_player_stats()` function from the `nflreadr` [@R-nflreadr] package.
