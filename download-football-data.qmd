...
This chapter provides an overview of how to download and process a wide range of football and fantasy football-related data.
We leverage the data in subsequent chapters for data analysis.
...
@HoDataDictionaries provide Data Dictionaries for the various National Football League (NFL) datasets at the following link: <https://nflreadr.nflverse.com/articles/index.html>.

## Types of NFL Data {#sec-downloadFootballDataOverview}

Below, we provide examples for how to download various types of NFL data using the `nflreadr` [@R-nflreadr] and `nflfastR` [@R-nflfastR] packages that are part of the `nflverse` ecosystem of packages [@R-nflverse].
...
If you have difficulty downloading the data files using the `nflreadr` [@R-nflreadr] or `nflfastR` [@R-nflfastR] packages, we also saved the data files so they are publicly available on the Open Science Framework: <https://osf.io/z6pg4>.
...
Guidance for how to merge the various NFL-related data files is provided by @SharpeNFLDataSets: <https://github.com/nflverse/nfldata/blob/master/DATASETS.md>.
The update and availability schedule for `nflverse` data is available online: <https://nflreadr.nflverse.com/articles/nflverse_data_schedule.html>.
...
load(file = "./data/nfl_actualStats_player_seasonal_raw.RData")
load(file = "./data/nfl_actualStats_player_weekly_raw.RData")
...
load(file = file.path(path, "/OneDrive - University of Iowa/Teaching/Courses/Fantasy Football/Data/nfl_rankings_raw.RData", fsep = ""))
...
#load(file = "./data/nfl_actualStats_player_career_raw.RData")
load(file = "./data/nfl_actualStats_weekly_raw.RData")
...
load(file = "./data/nfl_actualStats_player_seasonal.RData")
load(file = "./data/nfl_actualStats_player_weekly.RData")
...
You can access a data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_players
```

You can download the data using the following syntax:
...
Let's rearrange the data accordingly by player.

```{r}
nfl_players <- nfl_players_raw |> 
  select(gsis_id, everything()) |> # NOTE: the data structure changed in 2025: https://github.com/nflverse/nflverse-players?tab=readme-ov-file#whats-new-in-players-v2
  arrange(display_name)
```

In this case, I arrange the data by player using `display_name`.
You could arrange by `gsis_id` (or any other variable, for that matter), but `gsis_id` is fairly unintelligible by itself.
For instance, here are the first six players by ID according to `gsis_id`:

```{r}
head(nfl_players_raw$gsis_id)
```

Note how it is unclear who these players are until you combine this column with other, more relevant information.
Thus, I prefer to sort by a variable that is more interpretable, such as a player's name.
Here are the first six players by name according to `display_name`:
...
In data analysis, the variable(s) that are used to sort the dataframe are primarily chosen for aesthetic or usability purposes.
Many data analysis approaches do not depend on the order of the rows in the data.
However, the sorting of the dataframe may influence data wrangling operations.
Thus, when performing data wrangling operations it may be prudent to make sure to re-sort the data to the desired format before performing such operations.

Regardless of which variable(s) are used to sort the dataframe, it is important to know which variable(s) uniquely identify the rows because that will influence approaches to [merge](#sec-merging) the data with other dataframes.
For instance, it would be preferable to merge two data objects based on the player ID (`gsis_id`) rather than the player name (`display_name`), because multiple players share the same name, which can lead the [merge](#sec-merging) operation not to know which player from the first dataframe goes with which player from the second dataframe when both players have the same name (despite different having different player IDs).
...
```{r}
names(nfl_players) |> as.data.frame()
```
...
@HoFFPlayerIDs provide a Data Dictionary for fantasy player ID data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_ff_playerids.html>.
You can also access the data dictionary via `R` using the following syntax:
...
```{r}
names(nfl_playerIDs) |> as.data.frame()
```
...
- <https://nflreadr.nflverse.com/articles/dictionary_rosters.html> [@HoRosters]
- <https://github.com/nflverse/nfldata/blob/master/DATASETS.md#rosters> [@SharpeRosters]
...
nflreadr::dictionary_roster_status
...
```{r}
names(nfl_rosters) |> as.data.frame()

names(nfl_rosters_weekly) |> as.data.frame()
```
...
@HoSchedules provide a Data Dictionary for game schedules data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_schedules.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_schedules
```
...
```{r}
names(nfl_schedules) |> as.data.frame()
```
...
@SharpeStandings provides a Data Dictionary for standings data at the following link: <https://github.com/nflverse/nfldata/blob/master/DATASETS.md#standings>

We can determine NFL standings based on the `nfl_schedules` object using the `nflseedR` [@R-nflseedR] package.

```{r}
#| eval: false

nfl_standings_raw <- nflseedR::nfl_standings(
...
```{r}
names(nfl_standings) |> as.data.frame()
```
...
@HoCombine provide a Data Dictionary for data from the NFL Combine at the following link: <https://nflreadr.nflverse.com/articles/dictionary_combine.html>.
You can also access the data dictionary via `R` using the following syntax:
...
```{r}
names(nfl_combine) |> as.data.frame()
```
...
@HoDraftPicks provide a Data Dictionary for draft picks data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_draft_picks.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_draft_picks
```
...
Bo Jackson (1986, 1987), Mel Bratton (1988, 1989), and Craig Erickson (1991, 1992) were each drafted twice because they did not sign with the first team that drafted them.

Here are the variable names:

```{r}
names(nfl_draftPicks) |> as.data.frame()
```
...
@SharpeDraftValues provides a Data Dictionary for draft values data at the following link: <https://github.com/nflverse/nfldata/blob/master/DATASETS.md#draft_values>
...
```{r}
names(nfl_draftValues) |> as.data.frame()
```
...
@HoDepthCharts provide a Data Dictionary for data from weekly depth charts at the following link: <https://nflreadr.nflverse.com/articles/dictionary_depth_charts.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_depth_charts
```
...
Note: "Please note that the data source has changed after the 2024 season.
From 2025 onwards, depth charts are not assigned a week. Instead, each update provides a new ISO8601-formatted timestamp that users can assign to any point in time during the season.
Each update is appended to the existing data with a new timestamp." (<https://nflreadr.nflverse.com/articles/nflverse_data_schedule.html#depth-chart-data>).
...
nfl_depthCharts <- nfl_depthCharts_raw |> 
  select(gsis_id, season, week, depth_position, everything()) |> #NOTE: the data structure will change beginning in the 2025 season: https://nflreadr.nflverse.com/articles/dictionary_depth_charts.html
...
```{r}
names(nfl_depthCharts) |> as.data.frame()
```
...
@SharpeTrades provides a Data Dictionary for NFL trade data at the following link: <https://github.com/nflverse/nfldata/blob/master/DATASETS.md#trades>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_trades
```
...
nfl_trades_raw <- progressr::with_progress(
  nflreadr::load_trades(seasons = TRUE))
...
```{r}
names(nfl_trades) |> as.data.frame()
```
...
To download play-by-play data from prior weeks and seasons, we can use the `nflreadr::load_pbp()` function of the `nflreadr` package [@R-nflreadr].
We add a progress bar using the `progressr::with_progress()` function from the `progressr` package [@R-progressr] because it takes a while to run.
@HoPBP provide a Data Dictionary for the play-by-play data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_pbp.html>.
You can also access the data dictionary via `R` using the following syntax:
...
We use the `nfl4th` package [@R-nfl4th] to download fourth down data.
...
```{r}
names(nfl_4thdown) |> as.data.frame()
```
...
@HoParticipation provide a Data Dictionary for the participation data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_participation.html>.
You can also access the data dictionary via `R` using the following syntax:
...
nfl_participation_raw <- progressr::with_progress(
  nflreadr::load_participation(
    seasons = 2016:nflreadr::most_recent_season(), # participation data available after each season has ended per changelog for nflreadr v1.5.0 (https://cran.r-project.org/web/packages/nflreadr/news/news.html)
    include_pbp = TRUE))
```
...
nfl_participation_raw <- nflreadr::load_participation(
    seasons = 2016:nflreadr::most_recent_season(), # participation data available after each season has ended per changelog for nflreadr v1.5.0 (https://cran.r-project.org/web/packages/nflreadr/news/news.html)
    include_pbp = TRUE)
```
...
```{r}
names(nfl_participation) |> as.data.frame()
```
...
### Historical Actual Team Statistics {#sec-downloadActualTeamStats}

@HoTeamStats provide a Data Dictionary for historical actual team statistics data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_team_stats.html>.
You can also access the data dictionary via `R` using the following syntax:
...
nfl_actualStats_team_weekly_raw <- progressr::with_progress(
  nflreadr::load_team_stats(
    seasons = TRUE,
    summary_level = "week"))

nfl_actualStats_team_seasonal_reg_raw <- progressr::with_progress(
  nflreadr::load_team_stats(
    seasons = TRUE,
    summary_level = "reg"))

nfl_actualStats_team_seasonal_regPost_raw <- progressr::with_progress(
  nflreadr::load_team_stats(
    seasons = TRUE,
...
save(
  nfl_actualStats_team_weekly_raw,
  nfl_actualStats_team_seasonal_reg_raw,
  nfl_actualStats_team_seasonal_regPost_raw,
  file = "./data/nfl_actualStats_team_raw.RData"
)
```
...
load(file = "./data/nfl_actualStats_team_raw.RData")
```

The `nfl_actualStats_team_weekly_raw` object is in `season`-`week`-`team` form.
That is, each row should be uniquely identified by the combination of `season`, `week`, and `team`.
Let's rearrange the data accordingly:

```{r}
nfl_actualStats_team_weekly <- nfl_actualStats_team_weekly_raw |> 
  select(season, week, team, everything()) |> 
  arrange(season, week, team)
```

Let's check for duplicate `season`-`week`-`team` instances:

```{r}
nfl_actualStats_team_weekly |> 
...
```{r}
names(nfl_actualStats_team_weekly) |> as.data.frame()
```

The `nfl_actualStats_team_seasonal_reg_raw` and `nfl_actualStats_team_seasonal_regPost_raw` objects are in `season`-`team` form.
That is, each row should be uniquely identified by the combination of `season` and `team`.
Let's rearrange the data accordingly:

```{r}
nfl_actualStats_team_seasonal_reg <- nfl_actualStats_team_seasonal_reg_raw |> 
  select(season, team, everything()) |> 
  arrange(season, team)

nfl_actualStats_team_seasonal_regPost <- nfl_actualStats_team_seasonal_regPost_raw |> 
  select(season, team, everything()) |> 
  arrange(season, team)
```
...
nfl_actualStats_team_seasonal_regPost |> 
  group_by(season, team) |> 
  filter(n() > 1) |> 
  head()
```
...
save(
  ...
  nfl_actualStats_team_seasonal_reg,
  nfl_actualStats_team_seasonal_regPost,
  file = "./data/nfl_actualStats_team.RData"
)
```
...
We can download historical actual player statistics as a function of different timeframes, including [week-by-week](#sec-downloadActualWeeklyStats) and [season-by-season](#sec-downloadActualSeasonalStats).

#### Week-by-Week Statistics {#sec-downloadActualWeeklyStats}

We can download historical week-by-week actual player statistics using the `nflreadr::load_player_stats()` function from the `nflreadr` package [@R-nflreadr].
Nevertheless, we could also calculate historical actual week-by-week statistics, as in @sec-calculateActualWeeklyStats
@HoPlayerStats provide a Data Dictionary for statistics for offensive players at the following link: <https://nflreadr.nflverse.com/articles/dictionary_player_stats.html>.
@HoPlayerStatsDefense provide a Data Dictionary for statistics for defensive players at the following link: <https://nflreadr.nflverse.com/articles/dictionary_player_stats_def.html>.
You can also access the data dictionary via `R` using the following syntax:
...
nfl_actualStats_player_weekly_raw <- progressr::with_progress(
...
save(
  nfl_actualStats_player_weekly_raw,
  file = "./data/nfl_actualStats_player_weekly_raw.RData"
)
```
...
The `nfl_actualStats_weekly` object is in `player`-`season`-`week` form.
That is, each row should be uniquely identified by the combination of `player_id`, `season`, and `week`.
Let's rearrange the data accordingly:

```{r}
nfl_actualStats_player_weekly <- nfl_actualStats_player_weekly_raw |> 
...
```{r}
names(nfl_actualStats_player_weekly) |> as.data.frame()
```
...
save(
  nfl_actualStats_player_weekly,
  file = "./data/nfl_actualStats_player_weekly.RData"
)
```
...
We can download historical season-by-season actual player statistics using the `nflreadr::load_player_stats()` function from the `nflreadr` package [@R-nflreadr].
Nevertheless, we could also calculate historical actual season-by-season statistics, as in @sec-calculateActualSeasonalStats.
@HoPlayerStats provide a Data Dictionary for statistics for offensive players at the following link: <https://nflreadr.nflverse.com/articles/dictionary_player_stats.html>.
@HoPlayerStatsDefense provide a Data Dictionary for statistics for defensive players at the following link: <https://nflreadr.nflverse.com/articles/dictionary_player_stats_def.html>.
You can also access the data dictionary via `R` using the following syntax:
...
save(
  ...
  file = "./data/nfl_actualStats_player_seasonal_raw.Rdata"
)
```
...
The `nfl_actualStats_player_seasonal` objects are in `player`-`season` form.
That is, each row should be uniquely identified by the combination of `player_id` and `season`.
Let's rearrange the data accordingly:

```{r}
nfl_actualStats_player_seasonal_reg <- nfl_actualStats_player_seasonal_reg_raw |> 
  rename(team = recent_team) |> 
  select(player_id, season, everything()) |> 
  arrange(player_display_name, player_id, season)

nfl_actualStats_player_seasonal_regPost <- nfl_actualStats_player_seasonal_regPost_raw |> 
  select(player_id, season, everything()) |> 
  arrange(player_display_name, player_id, season)
```
...
```{r}
nfl_actualStats_player_seasonal_reg |> 
  group_by(player_id, season) |> 
  filter(n() > 1) |> 
  head()

nfl_actualStats_player_seasonal_regPost |> 
  group_by(player_id, season) |> 
  filter(n() > 1) |> 
  head()
```
...
```{r}
names(nfl_actualStats_player_seasonal_reg) |> as.data.frame()
```
...
save(
  nfl_actualStats_player_seasonal_reg, nfl_actualStats_player_seasonal_regPost,
  file = "./data/nfl_actualStats_player_seasonal.RData"
)
```
...
We calculate historical actual career statistics in @sec-calculateActualCareerStats.

### Injuries {#sec-injuries}

@HoInjuries provide a Data Dictionary for injury data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_injuries.html>.
You can also access the data dictionary via `R` using the following syntax:
...
Note: "Our data source died after the 2024 season.
At the moment, there is no 2025 data and there is no ETA yet as to when we will be able to make injury data available again." (<https://nflreadr.nflverse.com/articles/nflverse_data_schedule.html#injury-data>).
...
```{r}
names(nfl_injuries) |> as.data.frame()
```
...
@HoSnapCounts provide a Data Dictionary for snap counts data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_snap_counts.html>.
You can also access the data dictionary via `R` using the following syntax:
...
```{r}
names(nfl_snapCounts) |> as.data.frame()
```
...
ESPN provides Quarterbacks Ratings (QBR).
QBR is described by @Katz2016 (archived at <https://perma.cc/4TP6-E57U>).

@HoQBR provide a Data Dictionary for ESPN QBR data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_espn_qbr.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_espn_qbr
```
...
```{r}
names(nfl_espnQBR_seasonal) |> as.data.frame()

names(nfl_espnQBR_weekly) |> as.data.frame()
```
...
@HoNextGenStats provide a Data Dictionary for NFL Next Gen Stats data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_nextgen_stats.html>.
You can also access the data dictionary via `R` using the following syntax:
...
```{r}
names(nfl_nextGenStats_weekly) |> as.data.frame()
```
...
- <https://nflreadr.nflverse.com/articles/dictionary_pfr_passing.html> [@HoPFRPassing]
- <https://www.pro-football-reference.com/about/advanced_stats.htm> [@PFR2025\; archived at <https://perma.cc/LKH5-92PQ>]
...
nflreadr::dictionary_pfr_passing
```

@PFR2024 provides advanced stats from the 2024 season at the following link: <https://www.pro-football-reference.com/years/2024/advanced.htm>
...
Let's do some additional processing:

```{r}
# Drop players with missing values for pfr_id
nfl_advancedStatsPFR_seasonalByTeam <- nfl_advancedStatsPFR_seasonalByTeam |> 
  filter(!is.na(pfr_id))
```
...
nfl_advancedStatsPFR_weekly <- left_join(
  ...
  nfl_playerIDs |> 
    filter(!is.na(pfr_id)) |> 
    filter(gsis_id != "00-0039137") |> # drop DL Byron Young, keep OLB Byron Young
    select(pfr_id, gsis_id, position) |> 
    distinct(pfr_id, gsis_id, .keep_all = TRUE),
  by = "pfr_id"
) |> 
  rename(pos = position)
...
#nfl_advancedStatsPFR_weekly$gsis_id[which(nfl_advancedStatsPFR_weekly$gsis_id == "00-0035665" & nfl_advancedStatsPFR_weekly$team %in% c("TEN","MIA","DET"))] <- NA # drop LB David Young, keep DB David Young
...
#nfl_advancedStatsPFR_weekly$gsis_id[which(nfl_advancedStatsPFR_weekly$gsis_id == "00-0035292" & nfl_advancedStatsPFR_weekly$team %in% c("TEN","MIA","DET"))] <- NA # drop LB David Young, keep DB David Young
...
```{r}
names(nfl_advancedStatsPFR_seasonal) |> as.data.frame()

names(nfl_advancedStatsPFR_weekly) |> as.data.frame()
```
...
@HoContracts provide a Data Dictionary for player contracts data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_contracts.html>.
You can also access the data dictionary via `R` using the following syntax:
...
```{r}
names(nfl_playerContracts) |> as.data.frame()
```
...
@HoFTNCharting provide a Data Dictionary for FTN Charting data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_ftn_charting.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_ftn_charting
```
...
```{r}
names(nfl_ftnCharting) |> as.data.frame()
```
...
@HoFantasyProsRankings provide a Data Dictionary for FantasyPros ranking data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_ff_rankings.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_ff_rankings
```
...
nfl_rankings_raw <- progressr::with_progress(
...
nfl_rankings_raw <- nflreadr::load_ff_rankings(type = "all")
```
...
save(
  nfl_rankings_raw,
  file = "./data/nfl_rankings_raw.RData"
)
...
load(file = "./data/nfl_rankings_raw.RData")
```
...
The `nfl_rankings` object is in `player`-`page_type`-`fp_page`-`scrape_date` form.
That is, each row should be uniquely identified by the player's `id`, `page_type`, `fp_page`, and `scrape_date`.
Let's rearrange the data accordingly:

```{r}
nfl_rankings <- nfl_rankings_raw |> 
  select(id, page_type, fp_page, scrape_date, player, pos, team, everything()) |> 
  arrange(player, id, pos, page_type, fp_page, scrape_date)
```

Let's check for duplicate `player`-`page_type`-`fp_page`-`scrape_date` instances:

```{r}
nfl_rankings |> 
  group_by(id, page_type, fp_page, scrape_date) |> 
  filter(n() > 1) |> 
  head()
```
...
Let's check again for duplicate `player`-`page_type`-`fp_page`-`scrape_date` instances:
...
```{r}
names(nfl_rankings) |> as.data.frame()
```

The `nfl_rankings_draft` object is in `player`-`page_type` form.
That is, each row should be uniquely identified by the player's `id` and `page_type`.
Let's rearrange the data accordingly:
...
```{r}
names(nfl_rankings_draft) |> as.data.frame()
```
...
```{r}
names(nfl_rankings_weekly) |> as.data.frame()
```
...
@HoOpportunity provide a Data Dictionary for expected fantasy points data at the following link: <https://nflreadr.nflverse.com/articles/dictionary_ff_opportunity.html>.
You can also access the data dictionary via `R` using the following syntax:

```{r}
#| eval: false

nflreadr::dictionary_ff_opportunity
```
...
```{r}
names(nfl_expectedFantasyPoints_pbp) |> as.data.frame()
```
...
You can download fantasy football projections in `R` using the `ffanalytics::scrape_data()` function of the `ffanalytics` package [@R-ffanalytics].
Alternatively, if you prefer a graphical user interface, you can also download fantasy football projections using the Fantasy Football Analytics web application (depicted in @fig-projectionsTabFFA): <https://apps.fantasyfootballanalytics.net>.

::: {#fig-projectionsTabFFA}
![](images/projectionsTab.png){fig-alt="Fantasy Football Projections Available From Fantasy Football Analytics: <https://apps.fantasyfootballanalytics.net>."}

Fantasy Football Projections Available From Fantasy Football Analytics: <https://apps.fantasyfootballanalytics.net>.
:::
...
Below, we use the [scoring settings](https://support.espn.com/hc/en-us/articles/360003914032-Scoring-Formats) for fantasy leagues on [ESPN.com](https://www.espn.com):
...
#scoring_obj$rec$rec <- 1 # uncomment out if you have a PPR league
...
players_projections_seasonal_average <- players_projections_seasonal_average |> 
  add_ecr() |> # comment out if throws an error
  add_adp() |> 
  add_aav() |>
...
```{r}
names(dplyr::bind_rows(players_projectedPoints_seasonal)) |> as.data.frame()

names(players_projections_seasonal_average) |> as.data.frame()

names(dplyr::bind_rows(players_projectedPoints_weekly)) |> as.data.frame()

names(players_projections_weekly_average) |> as.data.frame()
```
...
We can compute historical actual player statistics as a function of different timeframes, including [week-by-week](#sec-calculateActualWeeklyStats), [season-by-season](#sec-calculateActualSeasonalStats), and the [player's career](#sec-calculateActualCareerStats).

#### Week-by-Week Statistics {#sec-calculateActualWeeklyStats}

We already load players' week-by-week statistics [above](#sec-downloadActualWeeklyStats) using the `nflreadr::load_player_stats()` function from the `nflreadr` [@R-nflreadr] package.
Nevertheless, we could compute players' weekly statistics from the play-by-play data using the `nflfastR::calculate_stats()` function from the `nflfastR` [@R-nflfastR] package:
...
```{r}
names(nfl_actualStats_weekly_player) |> as.data.frame()

names(nfl_actualStats_weekly_team) |> as.data.frame()
```
...
We already load players' season-by-season statistics [above](#sec-downloadActualSeasonalStats) using the `nflreadr::load_player_stats()` function from the `nflreadr` [@R-nflreadr] package.
Nevertheless, we could compute players' weekly statistics from the play-by-play data using the `nflfastR::calculate_stats()` function from the `nflfastR` [@R-nflfastR] package:
...
```{r}
names(nfl_actualStats_seasonal_player) |> as.data.frame()

names(nfl_actualStats_seasonal_team) |> as.data.frame()

names(nfl_actualStats_seasonal_player_inclPost) |> as.data.frame()

names(nfl_actualStats_seasonal_team_inclPost) |> as.data.frame()
```
...
Second, we can compute the players' career statistics using the `nflfastR::calculate_stats()` function from the `nflfastR` package [@R-nflfastR].
...
```{r}
names(nfl_actualStats_career_player) |> as.data.frame()

names(nfl_actualStats_career_player_inclPost) |> as.data.frame()
```
...
save(
  ...
  file = "./data/nfl_actualStats_player_career.RData"
)
```
...
We can then calculate players' historical actual fantasy points for [week-by-week](#sec-calculateActualPointsWeekly), [season-by-season](#sec-calculateActualPointsSeasonal), and the [player's career](#sec-calculateActualPointsCareer).
...
We set the `seasons` variable in @sec-downloadPlayByPlay.
...
```{r}
names(nfl_actualFantasyPoints_player_weekly) |> as.data.frame()

names(nfl_actualFantasyPoints_dst_weekly) |> as.data.frame()
```
...
```{r}
names(nfl_actualFantasyPoints_player_seasonal) |> as.data.frame()

names(nfl_actualFantasyPoints_dst_seasonal) |> as.data.frame()
```
...
```{r}
names(nfl_actualFantasyPoints_player_career) |> as.data.frame()
```
...
We calculate the player's age based on the difference between dates using the `lubridate` package [@R-lubridate]:
...
```{r}
names(player_stats_weekly) |> as.data.frame()
```
...
player_stats_seasonal <- dplyr::full_join(
  nfl_actualFantasyPoints_player_seasonal,
  nfl_players |> select(-position, -position_group, -latest_team),
  by = c("player_id" = "gsis_id")
)
...
```{r}
names(player_stats_seasonal) |> as.data.frame()
```
...
# Conclusion {#sec-downloadFootballDataConclusion}

This chapter provided an overview of how to download and process a wide range of football and fantasy football-related data.
Now that we have downloaded and processed a wide range of data, we can use the data in subsequent chapters for data analysis.
