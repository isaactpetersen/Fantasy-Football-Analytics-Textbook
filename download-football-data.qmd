...
To download play-by-play data from prior weeks and seasons, we can use the `load_pbp()` function of the `nflreadr` package [@R-nflreadr].
We add a progress bar using the `with_progress()` function from the `progressr` package [@R-progressr] because it takes a while to run.
...
We can download historical week-by-week actual player statistics using the `load_player_stats()` function from the `nflreadr` package [@R-nflreadr].
...
Second, we can compute the players' career statistics using the `calculate_stats()` function from the `nflfastR` package [@R-nflfastR].
...
We already load players' week-by-week statistics [above](#sec-downloadActualStats) using the `load_player_stats()` function from the `nflreadr` [@R-nflreadr] package.
Nevertheless, we could compute players' weekly statistics from the play-by-play data using the `calculate_stats()` function from the `nflfastR` [@R-nflfastR] package:
