...
bootstrappedFantasyPoints_summary %>% 
  dplyr::filter(position %in% c("DL","DT","DE")) %>% 
  mutate(
    across(
      where(is.numeric),
      \(x) round(x, digits = 2)))
...
bootstrappedFantasyPoints_summary %>% 
  dplyr::filter(position %in% c("LB","MLB","OLB")) %>% 
  mutate(
    across(
      where(is.numeric),
      \(x) round(x, digits = 2)))
...
bootstrappedFantasyPoints_summary %>% 
  dplyr::filter(position %in% c("DB","S","CB")) %>% 
  mutate(
    across(
      where(is.numeric),
      \(x) round(x, digits = 2)))
...
bootstrappedFantasyPoints_summary %>% dplyr::filter(mean > 100 & position == "QB") %>% arrange(CV) %>% head
bootstrappedFantasyPoints_summary %>% dplyr::filter(mean > 100 & position == "QB") %>% arrange(-CV) %>% head
```
...
ggplot2::ggplot(
  data = bootstrappedFantasyPoints %>%
    dplyr::filter(pos == "QB" & name == "Patrick Mahomes"),
  mapping = aes(
    x = projectedPoints)
) +
  geom_histogram(
...
ggplot2::ggplot(
  data = bootstrappedFantasyPoints %>%
    dplyr::filter(pos == "QB" & (name %in% c("Dak Prescott", "Drake Maye"))),
  mapping = aes(
    x = projectedPoints,
    group = name,
    #color = name,
...
You can generate a template for Monte Carlo simulations in the `SimDesign` [@R-SimDesign; @Chalmers2020] package using the following code:
