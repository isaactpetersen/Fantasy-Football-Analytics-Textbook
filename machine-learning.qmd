...
seasonalData <- powerjoin::power_full_join(
  ...
  filter(!is.na(season)) %>% 
  select(gsis_id, season, player_display_name, position, recent_team, games, everything()) #-todo: deduplicate team from prior merged objects
...
newData_seasonalQB <- left_join(
  newData_seasonalQB,
  newData_seasonal %>% select(gsis_id, player_display_name, recent_team),
  by = "gsis_id"
)
