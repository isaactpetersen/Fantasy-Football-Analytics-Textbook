...
We use the `future` [@R-future] package for parallel (faster) processing.
...
predict(
  smerf,
  X = data_test_qb %>% as_tibble() %>% dplyr::select(season:att_br.rush) %>% as.matrix(),
  Z = data_test_qb[,c("pacr")] %>% as.matrix(),
