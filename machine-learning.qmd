...
library("powerjoin")
library("tidymodels")
library("LongituRF") #to install: install.packages("https://cran.r-project.org/src/contrib/Archive/LongituRF/LongituRF_0.9.tar.gz") or remotes::install_github("sistm/LongituRF")
library("gpboost")
library("effectsize")
library("tidyverse")
library("knitr")
...
Machine learning is a class of algorithmic approaches that are used to identify patterns in data.
...
- supervised learning
    - continuous outcome (i.e., regression)
      ...
      - tree-based methods
        ...
        - [gradient tree boosting](#sec-treeBoosting)
...
We describe [factor analysis](#sec-factorAnalysis) in @sec-factorAnalysis.
...
### Ensemble Learing {#sec-machineLearningTypesEnsemble}
...
That way, 2025 fantasy points are in the same row as 2024 performance metrics, 2024 fantasy points are in the same row as 2023 performance metrics, and so on.
...
We fit various models to the training set based on which parameters we want to evaluate (e.g., how many trees to use in a [gradient tree boosting model](#sec-treeBoosting)).
...
[*k*-fold](#sec-kFoldCrossValidation) and [LOO](#sec-looCrossValidation) cross-validation are both forms of *internal* cross-validation.
...
LOO cross-validation is a special case of [*k*-fold cross-validation](#sec-kFoldCrossValidation) where $k = n$.
...
A random forest model combines the results of many decision trees.
A decision tree splits the data into groups based on the values of the predictors, and then predicts the outcome within each group.
A given decision tree can be noisy and prone to [overfitting](#sec-overfitting), so random forests build many trees, where each tree is trained on a random subset of the data and considers a random subset of predictors at each split.
The random forest then averages the predictions of all the decision trees, to improve overall predictive accuracy.
...
In the approaches below, using a random forest model (this section) and a [gradient tree boosting model](#sec-treeBoosting) (in @sec-treeBoosting), we address this by explicitly accounting for the nesting of longitudinal observations within players.
...
### Combining Gradient Tree Boosting with Mixed Models {#sec-treeBoosting}

To combine gradient tree boosting with [mixed models](#sec-mixedModels), we use the `gpboost` package [@R-gpboost].
Gradient tree boosting is similar to a [random forest model](#sec-randomForest).
Gradient tree boosting builds trees one by one, each trying to correct the errors of the previous trees.
By contrast, in a [random forest model](#sec-randomForest), all trees are built independently and then averaged.
This gradient tree boosting approach includes a [mixed model](#sec-mixedModels) to account for the longitudinal data nested within players.
...
#| fig-cap: "Predicted Versus Actual Fantasy Points for Gradient Tree Boosting Model."
#| fig-alt: "Predicted Versus Actual Fantasy Points for Gradient Tree Boosting Model."
...
We also compared them to simpler models, including a [regression model with one predictor](#sec-regressionOnePredictor) and a [regression model with multiple predictors](#sec-regressionMultiplePredictors) to determine the [incremental validity](#sec-incrementalValidity) of the machine learning models above and beyond the simpler models.
...
The most accurate model, in terms of [discrimination](#sec-discrimination) ($R^2$), had an $R^2$ near `{r} rsquare_maximum`, and it was achieved using a [random forest model](#sec-randomForest).
The next most accurate model, in terms of discrimination, was a [regression model with one predictor](#sec-regressionOnePredictor): the player's fantasy points in the previous season.
The [regression model with one predictor](#sec-regressionOnePredictor) was more accurate (in terms of $R^2$) than a [regression model with multiple predictors](#sec-regressionMultiplePredictors) and was just as accurate, if not more accurate, than other [machine learning models](#sec-fittingModels-machineLearning) that leveraged multiple predictors.
This suggests that the other predictors examined did not have much [incremental validity](#sec-incrementalValidity)—they did not show much utility above and beyond a player's previous fantasy points in predicting their future fantasy points.
If anything, considering the additional predictor variables showed *decremental* validity—the inclusion of the additional variables worsened prediction in several models, possibly by weakening the predictive association of the previous season's fantasy points and misattributing its predictive effect to other, related predictors in the model.
...
<!--
The most accurate models, in terms of [discrimination](#sec-discrimination) ($R^2$), had an $R^2$ near `{r} rsquare_maximum`, and it was achieved using several approaches—including a [gradient tree boosting model](#sec-treeBoosting) with many predictors and a [regression model with one predictor](#sec-regressionOnePredictor): the player's fantasy points in the previous season.
The [regression model with one predictor](#sec-regressionOnePredictor) was more accurate (in terms of $R^2$) than a [regression model with multiple predictors](#sec-regressionMultiplePredictors) and was just as accurate, if not more accurate, than other [machine learning models](#sec-fittingModels-machineLearning) that leveraged multiple predictors.
This suggests that the other predictors examined did not have [incremental validity](#sec-incrementalValidity)—they did not show utility above and beyond a player's previous fantasy points in predicting their future fantasy points.
If anything, considering the additional predictor variables showed *decremental* validity—the inclusion of the additional variables worsened prediction in several models, possibly by weakening the predictive association of the previous season's fantasy points and misattributing its predictive effect to other, related predictors in the model.
This is consistent with the common finding that simple models (and parsimony) are often preferable.
As an example, @Youngstrom2018 found that simple models did just as well and in some cases better than [LASSO models](#sec-lasso) in classifying bipolar disorder.
-->
...
The most accurate model in terms of [calibration](#sec-calibration) ([mean absolute error](#sec-meanAbsoluteError)) was the [random forest model](#sec-randomForest), which had a [mean absolute error](#sec-meanAbsoluteError) of `{r} mae_minimum`, and was just slightly more accurate than the [gradient tree boosting model](#sec-treeBoosting) and the [regression model with one predictor](#sec-regressionOnePredictor).
...
In sum, the [random forest model](#sec-randomForest), was the most accurate in terms of differentiating between players ($R^2$) and was also the most accurate in terms of how close the predicted values were to the actual values.

<!--
The most accurate model in terms of [calibration](#sec-calibration) ([mean absolute error](#sec-meanAbsoluteError)) was the [gradient tree boosting model](#sec-treeBoosting), which had a [mean absolute error](#sec-meanAbsoluteError) of `{r} mae_minimum`, and was just slightly more accurate than the [regression model with one predictor](#sec-regressionOnePredictor).
The [gradient tree boosting model](#sec-treeBoosting) being the most accurate suggests that it is important to account for the longitudinal data nested within players.
However, even in the most accurate model, the predicted values were over `{r} petersenlab::apa(mae_minimum, decimals = 0)` points away from the actual values, suggesting that the model-predicted values were not particularly accurate.
In sum, the [regression model with one predictor](#sec-regressionOnePredictor) and [gradient tree boosting model](#sec-treeBoosting) were the most accurate in terms of differentiating between players ($R^2$), whereas the [gradient tree boosting model](#sec-treeBoosting) was the most accurate in terms of how close the predicted values were to the actual values.
-->
...
<!-- The most accurate model, in terms of [discrimination](#sec-discrimination) ($R^2$), was achieved using a [regression model with one predictor](#sec-regressionOnePredictor): the player's fantasy points in the previous season—and a [gradient tree boosting model](#sec-treeBoosting).
The most accurate model in terms of [calibration](#sec-calibration) ([mean absolute error](#sec-meanAbsoluteError)) was the [gradient tree boosting model](#sec-treeBoosting), which accounted for the longitudinal nature of the data. -->
The most accurate model, in terms of [discrimination](#sec-discrimination) ($R^2$) and [calibration](#sec-calibration) ([mean absolute error](#sec-meanAbsoluteError)), was achieved using a [random forest model](#sec-randomForest).
The considerable unexplained variance suggests that we are missing key variables that are important predictors of future fantasy performance.
