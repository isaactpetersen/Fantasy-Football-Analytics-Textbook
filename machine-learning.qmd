...
Even if a learning rate of 1 did well in tuning, I use a lower learning rate (0.2) to avoid overfitting.
I also added some light regularization (`lambda_l2`) for better generalization.
I also set the maximum tree depth (`max_depth`) at 7 to capture up to 3-way interactions, and set the maximum number of terminal nodes (`num_leaves`) per tree at 2^5 (32).
I set the minimum number of samples in any leaf (`min_data_in_leaf`) to be 50.
