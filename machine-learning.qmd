...
We use a low learning rate (0.1) to avoid overfitting.
We set some light regularization (`lambda_l2`) for better generalization.
We also set the maximum tree depth (`max_depth`) at 5 to capture complex (up to 5-way) interactions, and set the maximum number of terminal nodes (`num_leaves`) per tree at 2^6 (64)—though the number will not reach greater than $2^\text{max_depth} = 2^5 = 32$.
