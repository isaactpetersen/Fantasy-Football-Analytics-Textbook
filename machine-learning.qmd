...
The most accurate model had an $R^2$ of `{r} rsquare_maximum`, and it was achieved using [regression with one predictor](#sec-regressionOnePredictor): the player's fantasy points in the previous season.
The [regression model with one predictor](#sec-regressionOnePredictor) was more accurate than a [regression model with multiple predictors](#sec-regressionMultiplePredictors) and [machine learning models](#sec-fittingModels-machineLearning) that leveraged multiple predictors.
...
Among the machine learning models, the [random forest model](#sec-randomForest) was most accurate.
