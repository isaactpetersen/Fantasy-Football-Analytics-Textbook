...
seasonalData$fantasyPoints_lag[is.na(seasonalData$fantasyPoints_lag)] <- 0
```
...
```{r}
seasonalData_subsetQB <- seasonalData_subset %>% 
  filter(position == "QB") %>% 
  select(
...
seasonalData_subsetRB <- seasonalData_subset %>% 
  filter(position == "RB") %>% 
  select(
    gsis_id, season, games, gs, years_of_experience, age, ageCentered20, ageCentered20Quadratic,
    height, weight, rookie_year, draft_number,
...
seasonalData_subsetWR <- seasonalData_subset %>% 
  filter(position == "WR") %>% 
  select(
    gsis_id, season, games, gs, years_of_experience, age, ageCentered20, ageCentered20Quadratic,
    height, weight, rookie_year, draft_number,
...
seasonalData_subsetTE <- seasonalData_subset %>% 
  filter(position == "TE") %>% 
  select(
    gsis_id, season, games, gs, years_of_experience, age, ageCentered20, ageCentered20Quadratic,
    height, weight, rookie_year, draft_number,
...
seasonalData_subsetQB_imp <- missForest::missForest(
  seasonalData_subsetQB %>% 
    select(ageCentered20, ageCentered20Quadratic, years_of_experience, passing_epa, pacr, dakota, rushing_epa) %>% 
    data.frame(),
  verbose = TRUE,
  mtry = 100,
  parallelize = "forests")
...
dataToImpute <- seasonalData_subset %>% 
  select(gsis_id, season, ageCentered20, ageCentered20Quadratic, fantasyPoints_lag, years_of_experience, passing_epa) %>% 
...
smerf <- LongituRF::MERF(
  ...
  Z = seasonalData_subsetQB_imp$ximp[,c("years_of_experience")] %>% as.matrix(),
