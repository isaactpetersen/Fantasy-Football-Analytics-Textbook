...
seasonalData_subset_imp <- missForest::missForest(
  seasonalData_subset %>% select(ageCentered20, ageCentered20Quadratic, fantasyPoints_lag) %>% data.frame,
  verbose = TRUE,
  mtry = 100,
  parallelize = "forests")
```
...
smerf <- LongituRF::MERF(
  X = seasonalData_subset_imp$ximp[,c("ageCentered20")] %>% as.matrix(),
  Y = seasonalData_subset_imp$ximp[,c("fantasyPoints_lag")],
  Z = seasonalData_subset_imp$ximp[,c("ageCentered20")] %>% as.matrix(),
