...
There are various ways to account for [base rates](#sec-baseRate).
One approach is to use [actuarial formulas](#sec-actuarialPrediction) (rather than human judgment) to make the predictions.
...
Another approach is to leverage Bayes' theorem, using Bayesian updating and the probability nomogram.

### Bayesian Updating {#bayesianUpdating}

Bayesian updating involves comparing the relative probability of two outcomes, $P(C | R)$ versus $P(\text{not } C | R)$.
...
As presented in @eq-bayes5, the posttest (or posterior) odds are equal to the pretest odds multiplied by the likelihood ratio.
Bayes' theorem is discussed in @sec-bayesTheorem.
Using this formula, and converting odds to probabilities, we can use a Fagan probability nomogram to determine the posttest probability following a test result.
The calculation of posttest probability is described in INSERT.
A *probability nomogram* is a way of visually applying [Bayes' theorem](#sec-bayesTheorem) to determine the posttest probability of having a condition based on the pretest (or prior) probability and likelihood ratio, as depicted in @fig-probabilityNomogram.
To use a probability nomogram, connect the dots from the starting probability (left line) with the likelihood ratio (middle line) to see the updated probability.
...
For instance, if the starting probability is 0.5% and the likelihood ratio is 10 (e.g., sensitivity = .90, specificity = .91: $\text{likelihood ratio} = \frac{\text{sensitivity}}{1 - \text{specificity}} = \frac{.9}{1-.91} = 10$) from a positive test, the updated probability is less than 5%, as depicted in @fig-probabilityNomogramLine.
The [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab] contains the `posttestProbability()` function that estimates the posttest probability of an event, given the pretest probability and the likelihood ratio, or given the pretest probability and the sensitivity and specificity of the test.
...
If the starting probability is 0.5% and the likelihood ratio is 0.11 from a negative test, the updated probability is essentially nearly indistinguishable from zero (0.05%).
```{r}
petersenlab::posttestProbability(
  pretestProb = .005,
...
The [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab] contains the `nomogrammer()` function that creates a nomogram plot, as adapted from Adam Chekroud (<https://github.com/achekroud/nomogrammer>):
